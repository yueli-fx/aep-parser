// showcase/booyah-clone/main.go — rebuild the entire "Booyah Glitch"
// project from scratch via our Go API, DAG leaf-first, the original as a read-only
// value oracle. See INDEX.md for scope and required reference inputs.
//
// Comps are assembled in DAG topological order (leaves first); each gen_<comp>.go
// adds one composition. Run from repo root: `go run ./showcase/booyah-clone`.
package main
...
const outPath = "showcase/booyah-clone/booyah-clone.aep"
