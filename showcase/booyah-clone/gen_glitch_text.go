// showcase/booyah-clone/gen_glitch_text.go — comp ⑩ "グリッチテキスト".
	...
	for _, of := range origLayer.Effects {
		...
		for _, suf := range effectScalarParams[mn] {
			...
			if len(op.Keyframes) > 0 {
				ok(serializer.AnimateEffectParam(newLayer, fx, mn+suf, scalarKfs(op.Keyframes)))
			} else if op.StaticValue != nil {
				ok(serializer.SetEffectParam(newLayer, fx, mn+suf, toScalar(op.StaticValue)))
			}
		}
		for _, suf := range effectVecParams[mn] {
			...
			if len(op.Keyframes) > 0 {
				ok(serializer.AnimateEffectParamVec(newLayer, fx, mn+suf, vecKfs(op.Keyframes)))
			} else if op.StaticValue != nil {
				ok(serializer.SetEffectParam(newLayer, fx, mn+suf, toFloats(op.StaticValue)))
			}
		}
		for _, suf := range effectExprParams[mn] {
			...
			pr := ok(serializer.SetEffectParam(newLayer, fx, mn+suf, 0.0))
