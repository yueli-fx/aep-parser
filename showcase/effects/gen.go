// showcase/effects/gen.go — from-scratch (no AE) showcase of the
...
//
// Run from the repo root: `go run ./showcase/effects`.
package main
...
const outPath = "showcase/effects/effects.aep"
...
// P is one SetEffectParam tweak identified by its parameter name
...
func main() {
	...
	cells := []cell{
		...
		{name: "02_GaussianBlur", col: 1, row: 0, effect: aep.EffectGaussianBlur, params: []P{{"Blurriness", 30.0}}},
		{name: "03_DropShadow", col: 2, row: 0, effect: aep.EffectDropShadow, params: []P{
			{"Shadow Color", []float64{255, 0, 0, 0}}, // shadow colour (black)
			{"Direction", 135.0},                      // direction
			{"Distance", 30.0},                        // distance
			{"Softness", 14.0},                        // softness
		}},
		{name: "04_Invert", col: 3, row: 0, effect: aep.EffectInvert},
		{name: "05_Tint", col: 0, row: 1, effect: aep.EffectTint, params: []P{{"Amount to Tint", 100.0}}}, // amount to tint
