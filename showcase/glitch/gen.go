// showcase/glitch/gen.go — from-scratch (no AE) PLUGIN-FREE glitch on
...
//
// Run from repo root: `go run ./showcase/glitch`.
package main
...
const outPath = "showcase/glitch/glitch.aep"
...
func main() {
	...
	must(err)
	set(mapL, fn, "Contrast", 320.0)      // Contrast (hard edges)
	set(mapL, fn, "Uniform Scaling", 0.0) // Uniform Scaling off
	set(mapL, fn, "Scale Width", 700.0)   // Scale Width (very wide)
	set(mapL, fn, "Scale Height", 9.0)    // Scale Height (thin bands)
	set(mapL, fn, "Complexity", 1.0)      // Complexity (blocky)
	...
	set(dispL, dm, "Max Horizontal Displacement", 26.0) // Max Horizontal Displacement
	set(dispL, dm, "Max Vertical Displacement", 0.0)    // Max Vertical (pure h-slices)

	glowL := fc.LayerByName("Glow")
	gl, err := aep.AddEffect(glowL, "ADBE Glo2")
	must(err)
	set(glowL, gl, "Glow Threshold", 70.0) // Glow Threshold (low -> strong bloom)
	set(glowL, gl, "Glow Radius", 42.0)    // Glow Radius
	set(glowL, gl, "Glow Intensity", 2.6)  // Glow Intensity
	...
	for _, m := range marks {
		...
		must(err)
		set(l, tr, "Uniform Scale", 0.0)  // Uniform Scale off
		set(l, tr, "Scale Width", 250.0)  // Scale Width %
		set(l, tr, "Scale Height", 250.0) // Scale Height %
		set(l, tr, "Position", m.pos)     // Position (comp fraction)

		ff, err := aep.AddEffect(l, "ADBE Fill")
		must(err)
		set(l, ff, "Color", m.col)
		must(l.SetBlendingMode(aep.BlendingModeAdd))
	}

	_, err = aep.AnimateEffectParam(dispL, dm, "Max Horizontal Displacement",
