...
regenerate: "go run ./showcase/essential-graphics  +  scripts/ae-worker/ae_run.ps1 verify.jsx"
---
...
**Color 控件须先 materialize**（已从本档移除）：color 参数默认被 elide(无值流),直接 expose 报错;须先 `SetEffectParam(layer, fx, "Color", []float64{...})` 物化值再提升。SetEffectParam 值类型是 `[]float64`(切片)非 `[4]float64`。
