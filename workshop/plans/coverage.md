...
- **Gradient XML (P2b 2C)**: `Property.Gradient *Gradient` + `GradientColorStop / GradientAlphaStop` 类型 — R only。parser 识别 "ADBE Vector Grad Colors" 属性的 `GCst → GCky → Utf8` chunk 路径，解 `prop.map version="4"` XML（color stops + alpha stops + version）。rifx 加 `IDGCst / IDGCky` 常量。fixture: `workshop/reference/py-aep/samples/models/property/gradient.aep`（py-aep 自带样本）。**剩余**：XML 重序列化 / SetGradient / per-keyframe gradients（gcky 多 Utf8）—— P2 followup（需 fixture）。
