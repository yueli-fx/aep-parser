...
- **PropertyGroup hierarchy (P2c)**: `AEPropertyGroup` 树 + `PropertyBase` interface — 把 tdgp 层级在 parser 时镜像出来，与 flat `Layer.Properties / Effects / Markers / Masks` 并存（叶子是同 `*Property` instance，pointer identity 保留）。Layer API: `PropertyTree()` / `PropertyGroupByMatchName(name)` / `PropertyByPath(...matchNames)` + 11 个 typed group accessor（`TransformGroup / AudioGroup / LayerStylesGroup / EffectsParade / MaskParade / TextPropertiesGroup / CameraOptionsGroup / LightOptionsGroup / MaterialOptionsGroup / GeometryOptionsGroup / ShapeContentsGroup`）。Group API: `Property(matchName) / Group(matchName) / ChildByIndex(i) / NumProperties / ParentGroup / PropertyByPath / PropertyIndex(child) / Depth()`。Leaf back-ref: `Property.ParentGroup()`。Effects/Marker subtree 出现在树里但不重解参数（用 Layer.Effects/Markers 取 typed）。**剩余**：`DefaultValue / LastValue / NbOptions / MinValue / MaxValue / UnitsText / PropertyControlType / PropertyValueType` 需 tdb4 byte RE 或 schema port，P2c followup。
