// Acceptance verify for from-scratch Text Rotation Animator (kinetic typography
// spin-in). Reads text_animator_rotation_args.json {input, done, resaved, png0,
// png1, png2}. Proves AE accepts the Go-built file, reads back the animator +
// Range Selector + Rotation value + the keyframed Range Offset (2 keys 0→100),
// resaves for the Go side, and renders three frames (t=0/1/2) so Go can assert
// on actual pixels that the single glyph's ink bounding box flips from tall to
// wide as the per-character rotation sweeps off it (red line 4 — render the
// visible surface). The layer text is a single asymmetric "L" so the bbox
// aspect ratio is an unambiguous orientation signal.
(function () {
    var argsFile = new File("e:/projects/tools/aep-parser/test_data/generated/args/text_animator_rotation_args.json");
    argsFile.open("r");
    ...
    try {
        ...
        for (var i = 1; i <= app.project.numItems; i++) {
            ...
            if (it instanceof CompItem && it.name === "TXROTGATE") { comp = it; break; }
        }
        if (!comp) { fail("comp TXROTGATE not found"); }
        else {
            ...
            if (!txt) { fail("TXT layer missing"); }
                ...
                if (!animators) { fail("no Text Animators group"); }
                    ...
                    if (animators.numProperties < 1) fail("no animator present");
                        ...
                        var rot = props.property("ADBE Text Rotation");
                        if (!rot) { fail("no Rotation property in animator"); }
                        else {
                            note("rotation value=" + rot.value);
                            if (Math.abs(rot.value - 90) > 0.5) fail("rotation=" + rot.value + ", want 90");
                        }
                    }
                }
                // Make the single glyph big and centred for a clean bbox sample.
                try {
