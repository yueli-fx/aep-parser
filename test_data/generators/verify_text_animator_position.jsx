// Acceptance verify for from-scratch Text Position Animator (kinetic typography
// slide-in). Reads text_animator_position_args.json {input, done, resaved,
// png0, png1, png2}. Proves AE accepts the Go-built file, reads back the
// animator + Range Selector + Position 3D offset + the keyframed Range Offset
// (2 keys 0→100), resaves for the Go side, and renders three frames (t=0/1/2)
// so Go can assert on actual pixels that the glyph block slides vertically as
// the displacement sweeps off the characters (red line 4 — render the visible
// surface).
(function () {
    var argsFile = new File("e:/projects/tools/aep-parser/test_data/generated/args/text_animator_position_args.json");
    argsFile.open("r");
    ...
    try {
        ...
        for (var i = 1; i <= app.project.numItems; i++) {
            ...
            if (it instanceof CompItem && it.name === "TXPOSGATE") { comp = it; break; }
        }
        if (!comp) { fail("comp TXPOSGATE not found"); }
        else {
            ...
            if (!txt) { fail("TXT layer missing"); }
                ...
                if (!animators) { fail("no Text Animators group"); }
                    ...
                    if (animators.numProperties < 1) fail("no animator present");
                        ...
                        note("selector name=" + sel.name + " match=" + sel.matchName);
                        ...
                        if (!pos) { fail("no Position 3D property in animator"); }
                        else {
                            note("position value=" + pos.value.toString());
                            // y component is the lift (we build it large & negative)
                            if (pos.value[1] > -1) fail("position y=" + pos.value[1] + ", want large negative");
                        }
                    }
                }
                // Place the text well inside the frame for pixel sampling (Go
                // can't set text-layer Position via its accessor; this is a
                // sampling fixture concern, not the capability under test). Done
                // AFTER readback so it can't block it.
                try {
                    var posProp = txt.property("ADBE Transform Group").property("ADBE Position");
                    note("pos before=" + posProp.value.toString());
                    posProp.setValue([300, 480]);
