// test_data/generators/verify_v2_2_stroke.jsx
//
// V2.2.1 focused Stroke ship-gate — opens an .aep with NewShapeLayer + AddRect
// (geometry) + AddStroke (DISTINCT Color/Width/Opacity, not the embed fixture's
// values) + asserts AE accepted the stroke (no silent-drop), then re-saves so
// the Go side decodes the re-saved Color/Width/Opacity cdat. (ExtendScript
// shape .value is unreliable; verify via re-save like the ellipse/path gates.)
//
// args.json fixed path: test_data/v2_2_stroke_args.json — {"input","done","resaved"}

(function () {
    var argsFile = new File("e:/projects/tools/aep-parser/test_data/generated/args/v2_2_stroke_args.json");
    var args = {};
    var doneFile = null;
    var log = [];
    var ok = false;
    ...
    try {
        ...
        var L = layerByName(c, "Stroke_Static");
        var checks = [
            check("layer count 1", c.layers.length === 1),
            check("Stroke_Static layer exists (not dropped)", L !== null)
        ];
        if (L) {
            var root = L.property("ADBE Root Vectors Group");
            var rect = findByMatchName(root, "ADBE Vector Shape - Rect");
            var stroke = findByMatchName(root, "ADBE Vector Graphic - Stroke");
    ...
    try {
        if (!doneFile) doneFile = new File("e:/projects/tools/aep-parser/test_data/v2_2_stroke_test.done");
        doneFile.open("w");
