// Acceptance verify for from-scratch Text Animator (kinetic typography reveal).
// Reads text_animator_args.json {input, done, resaved, png0, png1, png2}.
// Proves AE accepts the Go-built file, reads back the animator + Range Selector
// + Opacity + the keyframed Range Offset (2 keys 0→100), resaves for the Go
// side, and renders three frames (t=0/1/2) so Go can assert on actual pixels
// that the text sweeps from fully hidden → fully revealed (red line 4 — render
// the capability's visible surface).
(function () {
    var argsFile = new File("e:/projects/tools/aep-parser/test_data/generated/args/text_animator_args.json");
    argsFile.open("r");
    ...
    try {
        ...
        for (var i = 1; i <= app.project.numItems; i++) {
            ...
            if (it instanceof CompItem && it.name === "TXANIMGATE") { comp = it; break; }
        }
        if (!comp) { fail("comp TXANIMGATE not found"); }
        else {
            ...
            if (!txt) { fail("TXT layer missing"); }
                ...
                if (!animators) { fail("no Text Animators group"); }
                    ...
                    if (animators.numProperties < 1) fail("no animator present");
                        ...
                        if (!op) { fail("no Opacity property in animator"); }
                        else {
                            note("opacity value=" + op.value);
                            if (Math.abs(op.value - 0) > 0.5) fail("opacity=" + op.value + ", want 0");
                        }
                    }
                }
                // Place the text well inside the frame for pixel sampling (the
                // Go side can't set text-layer Position via its accessor;
                // positioning is a sampling fixture concern, not the capability
                // under test). Done AFTER the readback so it can't block it.
                try {
                    var posProp = txt.property("ADBE Transform Group").property("ADBE Position");
                    note("pos before=" + posProp.value.toString());
                    posProp.setValue([300, 380]);
