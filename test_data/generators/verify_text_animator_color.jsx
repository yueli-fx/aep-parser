// Acceptance verify for from-scratch Text Fill Color Animator (kinetic
// typography colour wipe). Reads text_animator_color_args.json {input, done,
// resaved, png0, png1, png2}. Proves AE accepts the Go-built file, reads back
// the animator + Range Selector + Fill Color value + the keyframed Range Offset
// (2 keys 0→100), forces the base text colour to white (so the animator's red
// override is an unambiguous colour signal), resaves for the Go side, and
// renders three frames (t=0/1/2) so Go can assert on actual pixels that the
// glyphs' ink colour sweeps from red (selected) to white (deselected) as the
// Range Offset slides the selection off the text (red line 4 — render the
// visible surface).
(function () {
    var argsFile = new File("e:/projects/tools/aep-parser/test_data/generated/args/text_animator_color_args.json");
    argsFile.open("r");
    ...
    try {
        ...
        for (var i = 1; i <= app.project.numItems; i++) {
            ...
            if (it instanceof CompItem && it.name === "TXCOLGATE") { comp = it; break; }
        }
        if (!comp) { fail("comp TXCOLGATE not found"); }
        else {
            ...
            if (!txt) { fail("TXT layer missing"); }
                ...
                if (!animators) { fail("no Text Animators group"); }
                    ...
                    if (animators.numProperties < 1) fail("no animator present");
                        ...
                        if (!fc) { fail("no Fill Color property in animator"); }
                        else {
                            var v = fc.value;
                            note("fillColor value=" + v.toString());
                            if (Math.abs(v[0] - 1) > 0.02 || Math.abs(v[1] - 0) > 0.02 || Math.abs(v[2] - 0) > 0.02) {
                                fail("fillColor=" + v.toString() + ", want [1,0,0,*] (red)");
                            }
                        }
                    }
                }
                // Force base text colour white + a readable size so the red
                // override reads as an unambiguous per-character colour signal.
                try {
