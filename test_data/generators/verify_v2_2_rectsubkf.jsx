// test_data/generators/verify_v2_2_rectsubkf.jsx — V2.2.1 子项⑦ Rect Position + Roundness
// keyframe gate. Asserts AE accepts an animated Rect Position (spatial Vec2,
// bpk-104) + Roundness (1D, bpk-48) and reads back values, then re-saves for
// Go-side keyframe decode. args: v2_2_rectsubkf_args.json.
(function () {
    var argsFile = new File("e:/projects/tools/aep-parser/test_data/generated/args/v2_2_rectsubkf_args.json");
    ...
    try {
        ...
        var L = layerByName(c, "RectSub");
        var checks = [check("1 layer", c.layers.length === 1), check("RectSub exists", L !== null)];
        if (L) {
            var rect = findBy(L.property("ADBE Root Vectors Group"), "ADBE Vector Shape - Rect");
            checks.push(check("Rect present", rect !== null));
            if (rect) {
                var pos = rect.property("ADBE Vector Rect Position");
                var rnd = rect.property("ADBE Vector Rect Roundness");
                checks.push(check("Position 2 kf", pos.numKeys === 2));
                checks.push(check("Roundness 2 kf", rnd.numKeys === 2));
                if (pos.numKeys === 2) {
                    var p1 = pos.keyValue(2); log.push("pos kf1=" + p1.toString());
                    checks.push(check("pos X~40", Math.abs(p1[0] - 40) < 0.5));
                    checks.push(check("pos Y~50", Math.abs(p1[1] - 50) < 0.5));
                }
                if (rnd.numKeys === 2) {
                    var r1 = rnd.keyValue(2); log.push("rnd kf1=" + r1.toString());
                    checks.push(check("roundness~20", Math.abs(r1 - 20) < 0.5));
                }
            }
        }
    ...
    try { if (!doneFile) doneFile = new File("e:/projects/tools/aep-parser/test_data/v2_2_rectsubkf_test.done"); doneFile.open("w"); doneFile.write((ok ? "PASS\n" : "FAIL\n") + log.join("\n")); doneFile.close(); } catch (e2) {}
