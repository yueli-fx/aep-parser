...
type ProjectWriter interface {
	...
	// RIFX tree. The scene Project.WriteAEP delegates here so the sync free
	// functions (which reach concrete back-refs) stay in the serializer stage.
	WriteAEP(p *Project, w io.Writer) error
...
type LayerWriter interface {
	...
	// (@0x6C) pair so the scene SetStretch accessor mirrors the written ratio
	// without naming the concrete back-ref. BtdsData exposes the live btds
	// chunk bytes so the scene text resync re-decodes after a back-side splice.
	StretchFrac() (float64, bool)
	BtdsData() []byte

	// LdtaRaw exposes the live ldta chunk bytes so the scene LdtaRawBytes
	// accessor surfaces them without naming the concrete back-ref.
	LdtaRaw() []byte
}
...
type KeyframeWriter interface {
	...
	// FrameRateHz exposes the owning composition's FrameRate cached at parse
	// time so the scene FrameTime / SetFrameTime accessors avoid naming the
	// concrete back-ref.
	FrameRateHz() float64

	// CompTickRate exposes the owning composition's TickRate cached at parse
	// time so the scene SetTime accessor recomputes the quantized Keyframe.Time
	// without naming the concrete back-ref.
	CompTickRate() float64
}
...
type MarkerWriter interface {
	...
	// CompTickRate exposes the owning composition's TickRate cached at parse
	// time so the scene SetTime accessor recomputes the quantized Marker.Time
	// without naming the concrete back-ref.
	CompTickRate() float64
}
...
// MarkerSetRef is the back-ref handle for a Marker's owning marker-set
// container (the shared lhd3 / ldat / mrky chunks behind one "ADBE Marker"
// set). The structural marker ops (RemoveMarker / AddMarker) are serializer
// free-functions reaching the concrete *markerList via markerSet; this nominal
// marker keeps the scene Marker.list field interface-typed and chunk-free.
type MarkerSetRef interface {
	IsMarkerSetRef()
}
