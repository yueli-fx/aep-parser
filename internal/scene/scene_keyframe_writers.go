...
//
//aep:cap domain=keyframe tier=stable verify=roundtrip boundary="length-preserving(ldat tick write);chunk size 不变;无专门 AE gate→round-trip" alias="keyframe time,关键帧时间,kf time,时间点"
...
//
//aep:cap domain=keyframe tier=stable verify=roundtrip boundary="length-preserving;1D 传 float64,N-D 传 []float64 且长度须匹配;无专门 AE gate→round-trip" alias="keyframe value,关键帧值,kf value,属性值"
...
//
//aep:cap domain=keyframe tier=stable verify=roundtrip boundary="length-preserving(1B @0x04);无专门 AE gate→round-trip" alias="keyframe interpolation,关键帧插值,in interpolation,缓入缓出"
...
//
//aep:cap domain=keyframe tier=stable verify=roundtrip boundary="length-preserving(1B @0x05);无专门 AE gate→round-trip" alias="keyframe out interpolation,关键帧输出插值,out interpolation,贝塞尔线性"
...
//
//aep:cap domain=keyframe tier=stable verify=roundtrip boundary="length-preserving;slice 长度须匹配 ease shape(空间属性 1 / N-D 为 N);无专门 AE gate→round-trip" alias="temporal ease,时间缓动,ease in,速度曲线,缓入"
...
//
//aep:cap domain=keyframe tier=stable verify=roundtrip boundary="length-preserving;slice 长度须匹配 ease shape;无专门 AE gate→round-trip" alias="temporal ease out,缓出,ease out,速度曲线出"
...
//
//aep:cap domain=keyframe tier=stable verify=roundtrip boundary="length-preserving(dims×8B);仅空间属性有效(Position/AnchorPoint);slice 长度须等于 Property.Components;无专门 AE gate→round-trip" alias="spatial tangent,空间切线,bezier tangent,运动路径切线,in tangent"
...
//
//aep:cap domain=keyframe tier=stable verify=roundtrip boundary="length-preserving(dims×8B);仅空间属性有效;slice 长度须等于 Property.Components;无专门 AE gate→round-trip" alias="out tangent,出切线,bezier out tangent,运动路径出切线"
