...
//   - "py-aep" = py-aep/binary/composition_chunks.py CdtaChunk
//   - "fixture" = hex-dumped from test_data/re_tickrate.aep
//   - "local" = inferred from observation only, no external source
//
// Offsets used here are big-endian unless otherwise noted.
//
//	0x08–0x0B : internal_timebase  (uint32) — modern: ticks/sec; legacy: see deriveTickRate           [py-aep + fixture]
//	0x1C–0x1F : work_area_start_dividend (uint32)                                                     [py-aep + fixture]
//	0x20–0x23 : work_area_start_divisor  (uint32)                                                     [py-aep + fixture]
//	0x24–0x27 : work_area_end_dividend   (uint32) — 0xFFFFFFFF = "extend to comp Duration"           [py-aep + fixture; sentinel = local]
//	0x28–0x2B : work_area_end_divisor    (uint32)                                                     [py-aep + fixture]
//	0x34      : bg_color_r (uint8)                                                                    [py-aep + fixture]
//	0x35      : bg_color_g (uint8)                                                                    [py-aep + fixture]
//	0x36      : bg_color_b (uint8)                                                                    [py-aep + fixture]
...
//	0xAE–0xAF : shutter_angle (uint16, degrees; AE UI default 180)                                    [py-aep + fixture]
//	0xB0–0xB3 : DurationFrames (uint32)                                                                [local + fixture]
//	0xB4–0xB7 : shutter_phase (int32 — likely degrees, exact units not UI-verified)                   [py-aep + fixture]
