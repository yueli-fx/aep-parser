...
var embeddedDummyCompTemplate2020 []byte

//go:embed templates/2022_dummy_comp.aep
var embeddedDummyCompTemplate2022 []byte

//go:embed templates/2025_dummy_comp.aep
var embeddedDummyCompTemplate2025 []byte

// compTemplate carries per-target builder chunk references extracted from
// that target's dummy_comp.aep. Each target needs its own copy because
// AE versions diverge in Item internals (e.g. AE 24+ adds 10 Material/
// Lighting property groups; ldta length 160→164B).
...
var (
	...
	compTemplates map[AETarget]*compTemplate
)
...
// ensureCompTemplates lazy-init per-target template-copy chunks. AE 2020 ↔
// AE 2025 diverge in Item internals (AE 24+ added 10 Material/Lighting
// property groups; ldta grew 4 bytes); patching is brittle, so each target
// gets a comp template generated by that actual AE version.
//
// Sources (all generated via tmp_debug/gen_dummy_comp.jsx run against each AE):
//   - templates/2020_dummy_comp.aep — AE 2020 (17.7) — FEE 0 children, ldta 160B, tdgp 19
//   - templates/2022_dummy_comp.aep — AE 2022 (22.6) — FEE 1 child, ldta 160B, tdgp 19 (hybrid)
//   - templates/2025_dummy_comp.aep — AE 2025         — FEE 1 child, ldta 164B, tdgp 37 (Material features)
func ensureCompTemplates() {
	templateInit.Do(func() {
		compTemplates = map[AETarget]*compTemplate{
			TargetAE2020: loadCompTemplate(embeddedDummyCompTemplate2020),
			TargetAE2022: loadCompTemplate(embeddedDummyCompTemplate2022),
			TargetAE2025: loadCompTemplate(embeddedDummyCompTemplate2025),
		}
	})
}
...
// templateFor returns the comp template for a target, falling back to AE 2025
// if the target's template wasn't embedded (forward compat: future AETarget
// values get the latest template until a dedicated one is RE'd).
func templateFor(target AETarget) *compTemplate {
	ensureCompTemplates()
	if t, ok := compTemplates[target]; ok {
		return t
	}
	return compTemplates[TargetAE2025]
}
