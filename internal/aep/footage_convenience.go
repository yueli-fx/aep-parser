...
func (f *Footage) FootageMissing() bool {
	...
	if f.sspcChunk == nil || len(f.sspcChunk.Data) <= sspcOffFootageMissing {
		return false
	}
	return f.sspcChunk.Data[sspcOffFootageMissing] != 0
}
...
func (f *Footage) HasAudio() bool {
	...
	if f.sspcChunk == nil || len(f.sspcChunk.Data) < sspcOffAudioSampleRate+8 {
		return false
	}
	bits := binary.BigEndian.Uint64(f.sspcChunk.Data[sspcOffAudioSampleRate : sspcOffAudioSampleRate+8])
...
func (f *Footage) StartFrame() int {
	if f.sspcChunk == nil || len(f.sspcChunk.Data) < sspcOffStartFrame+4 {
		return 0
	}
	return int(binary.BigEndian.Uint32(f.sspcChunk.Data[sspcOffStartFrame : sspcOffStartFrame+4]))
}

// EndFrame returns the footage end frame (sspc @0xB0, uint32 BE).
// 0 for non-sequence footage and for fixtures without a full-size sspc.
func (f *Footage) EndFrame() int {
	if f.sspcChunk == nil || len(f.sspcChunk.Data) < sspcOffEndFrame+4 {
		return 0
	}
	return int(binary.BigEndian.Uint32(f.sspcChunk.Data[sspcOffEndFrame : sspcOffEndFrame+4]))
}
