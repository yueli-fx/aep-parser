...
var fieldValidation = map[string]FieldMeta{
	...
	"comps[].layers[].text_animators[].property": {
		...
		Enum:       []string{"opacity", "position", "scale", "rotation", "color", "tracking", "character_offset", "fill_opacity", "stroke_opacity"},
	},
	"comps[].layers[].text_animators[].value": {
		Validation: "Opacity, rotation, tracking, character_offset, fill_opacity, and stroke_opacity values must be numbers; position and scale values must be 3-number arrays; color values must be 3- or 4-number arrays.",
	},
