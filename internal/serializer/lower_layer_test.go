...
func TestLowerShapeLayer_EmptyHasLayrChunk(t *testing.T) {
	base := &Layer{Type: LayerTypeShape, Name: "S1"}
	s := WrapShapeLayer(base)
	chunk, err := LowerShapeLayerForTest(s)
	if err != nil {
		t.Fatal(err)
	}
	if chunk == nil {
...
func TestLowerShapeLayer_LdtaSizeByTarget(t *testing.T) {
	...
	for _, tc := range cases {
		base := &Layer{Type: LayerTypeShape, Name: "S1", ID: 13}
		s := WrapShapeLayer(base)
		chunk, err := LowerShapeLayerForTargetForTest(s, tc.target)
		if err != nil {
			t.Fatal(err)
		}
...
func TestLowerShapeLayer_WithShape_HasRootVectorsGroup(t *testing.T) {
	base := &Layer{Type: LayerTypeShape, Name: "S1"}
	s := WrapShapeLayer(base)
	s.RootGroup().Children = append(s.RootGroup().Children, NewRectNode())
	chunk, err := LowerShapeLayerForTest(s)
	if err != nil {
		t.Fatal(err)
	}
