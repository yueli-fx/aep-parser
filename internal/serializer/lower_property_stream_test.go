...
func TestLowerFloat64Stream_Static(t *testing.T) {
	...
	chunk, err := LowerFloat64Stream(ps, "ADBE Opacity", "Opacity", NewLowerCtxForTest())
	if err != nil {
		t.Fatal(err)
	}
	if chunk == nil {
...
func TestLowerVec2Stream_Animated(t *testing.T) {
	...
	chunk, err := LowerVec2Stream(ps, "ADBE Position", "Position", NewLowerCtxForTest())
	if err != nil {
		t.Fatal(err)
	}
	if chunk == nil {
...
func TestLowerColorStream_Static_CdatSize(t *testing.T) {
	...
	chunk, err := LowerColorStream(ps, "ADBE Vector Fill Color", "Color", NewLowerCtxForTest())
	if err != nil {
		t.Fatal(err)
	}
	tdbs := chunk.FindFirstList(rifx.IDTdbs)
...
func TestLowerPathStream_Static_EmitsOmS(t *testing.T) {
	ps := codec.NewPropertyStream[BezierPath]()
	if err := ps.SetStaticValue(BezierPath{
	...
	chunk, err := LowerPathStream(ps, "ADBE Vector Shape", "Path", NewLowerCtxForTest())
	if err != nil {
		t.Fatal(err)
	}
	if chunk == nil {
