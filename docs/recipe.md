...
| `project.target_version` | `string` | - | AE target template used when compiling the recipe. | Must be AE2020, AE2021, AE2022, AE2023, AE2024, or AE2025. Omitted target_version defaults to AE2020. | `project.create_target` (`NewProject`) | examples/recipes/minimal-adjustment-layer.json |
...
| `comps[].layers[].text_style.horizontal_scale` | `number` | - | Layer text style horizontal text scale. | Text horizontal scale must be non-negative. | `text.set_run_horizontal_scale` (`Layer.SetRunHorizontalScale`) | examples/recipes/minimal-text-style.json |
| `comps[].layers[].text_style.vertical_scale` | `number` | - | Layer text style vertical text scale. | Text vertical scale must be non-negative. | `text.set_run_vertical_scale` (`Layer.SetRunVerticalScale`) | examples/recipes/minimal-text-style.json |
...
| `comps[].layers[].shape.fill_color` | `array<float64>` | - | Solid fill color as RGB channels. | Fill color must contain RGB or RGBA channels in the 0..255 range. | `shape.fill.set_color` (`FillNode.SetColor`) | examples/recipes/minimal-effect-layer-param.json |
