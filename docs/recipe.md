...
| `comps[].layers[].text_animators[]` | `array<recipe.TextAnimatorSpec>` | - | Text animator definitions. | - | `text_animator.add_opacity` (`AddTextOpacityAnimator`)<br>`text_animator.add_position` (`AddTextPositionAnimator`)<br>`text_animator.add_scale` (`AddTextScaleAnimator`) | examples/recipes/minimal-text-animator-opacity.json |
...
| `comps[].layers[].text_animators[].value` | `any` | - | Text animator property value. | Opacity value must be a number; position and scale values must be 3-number arrays. | - | examples/recipes/minimal-text-animator-opacity.json |
