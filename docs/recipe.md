...
| `comps[].layers[].text_animators[].value` | `any` | - | Text animator property value. | Opacity, rotation, tracking, and character_offset values must be numbers; position and scale values must be 3-number arrays; color values must be 3- or 4-number arrays. | - | examples/recipes/minimal-text-animator-character-offset.json |
