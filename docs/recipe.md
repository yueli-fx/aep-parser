...
| `comps[].layers[].text_animators[]` | `array<recipe.TextAnimatorSpec>` | - | Text animator definitions. | - | `text_animator.add_opacity` (`AddTextOpacityAnimator`)<br>`text_animator.add_position` (`AddTextPositionAnimator`)<br>`text_animator.add_scale` (`AddTextScaleAnimator`)<br>`text_animator.add_rotation` (`AddTextRotationAnimator`)<br>`text_animator.add_color` (`AddTextColorAnimator`) | examples/recipes/minimal-text-animator-color.json |
...
| `comps[].layers[].text_animators[].value` | `any` | - | Text animator property value. | Opacity and rotation values must be numbers; position and scale values must be 3-number arrays; color values must be 3- or 4-number arrays. | - | examples/recipes/minimal-text-animator-color.json |
