...
Serializes the project to memory (WriteAEP) and re-parses the

bytes (FromReader), returning a fresh Project. The receiver is left untouched; callers switch to the returned project and re-resolve item / layer handles (by name or ID — IDs are preserved by the round-trip).
...
Returns an empty Project parsed from the embedded AE skeleton

matching the requested target. Zero args = TargetAE2020 (maximum compatibility); pass at most one target. Subsequent NewComposition calls populate it.
...
Mirrors ExtendScript RenderQueue.items.add(comp). Alpha /
...
Mirrors ExtendScript RenderQueueItem.remove(). Alpha /

structural. Free function (not a method) so the implementation can live in internal/serializer; the aep facade re-exports it.
