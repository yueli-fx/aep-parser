...
跟 [camera-filmsize-ldta-write-blocked](../composition/camera-filmsize-ldta-write-blocked.md) / [variable-fonts-write-noop](../text/variable-fonts-write-noop.md) 同族（AE 持久化但 ScriptingAPI 隐藏），但**结论相反**：那两个是写不动，这个**写得动**——见下。
...
- **接受**：JSX 只断言 `app.open` 不抛 + `numItems>=1`，PASS = 打开成功（排除 mode 1/2/3 损坏，见 [ae25-acceptance-gate](../workflow/ae25-acceptance-gate.md)）。
