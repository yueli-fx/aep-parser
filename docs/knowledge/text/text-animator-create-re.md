...
- structural op（Remove/Duplicate/Move 对 text animators）走 `mutate_property_structural.go`，
  **2026-06-16 双版本 ship-gate PASS**（`text_animator_struct_shipgate_test.go`：3 动画器
  ...
  值辨识 driven leaf）。详 `[property-indexed-group-structural-re](../property/property-indexed-group-structural-re.md)` 落地状态。
...
——SetExpression 早经 S2 + 2026-06-15 Utf8-order 修复渲染验证（含 effect param，见 [expression-enable-byte-pair](../keyframe-expression/expression-enable-byte-pair.md)）。
...
- 2026-06-22(2) **companion-leaf 必须物化，否则 AE UI 编辑/删层炸**（comp ② 真机 review 追修，commit 后续）：
  ...
  **修法**：gen jsx 显式 `addProperty` 伴生 + setValue 原值(1) → 物化进模板（2042→2316/2590B）；facade
  ...
  `maxLayerIDInItemList+1` 分层 ID，两个 comp 的首个用户层都拿 ID 13 → **跨 comp ID 撞号**（[nextitemid-must-include-layer-ids](../parse-serialize/nextitemid-must-include-layer-ids.md)
