...
AE 软件路径按当前仓库自动化约定走 `scripts/ae-worker/ae_run.ps1`。）
...
[kerning-first-enable](kerning-first-enable.md)（2026-05-19）说「无 scripting API」是该 API 出现前的旧认知。
...
3. **`re_text.aep` 有历史累积的重复 RE_TEXT comp**（老 JSX 无 fresh_project
   守卫，[jsx-state-leak](../workflow/jsx-state-leak.md) 活例）：按名字找层会命中多个，测试须按 parse 序
   固定取首个，否则「改了最后一个、验了第一个」假阴。

## 相关

- [kerning-first-enable](kerning-first-enable.md) — btdk 内 structural-add 被拒、splice 可行的先例
- [camera-light-layer-create-re](../layer/camera-light-layer-create-re.md) — embed-whole-Layr 创建机制（NewTextLayer 同族）
