...
- ship-gate：`selection_both_layers`(AE2025, 2 图层)→2020 经 `scripts/ae-worker/ae_run.ps1` 开 AE 2020 → 4 items / Comp 1 / 2 层同名同序，无"损坏/跳过"，**工具主流路径(2025→2020)产出 AE 视为原生且内容无损**。AE 自动化 prefs 注意：脚本跑完让 AE 优雅 `app.quit()` 等几秒再 kill，否则首选项损坏（见 [ae-automation-occlusion-crashstate.md](../workflow/ae-automation-occlusion-crashstate.md)）。
