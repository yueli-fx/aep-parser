...
```
booyah ⑩ 实测：全 131 个 mask 都是 4 顶点 Add-mode 轴对齐矩形，bbox round-trip `maxErr=0`，双版本 AE 接受不 drop。代码见 `showcase/booyah-clone/gen_glitch_text.go` `maskBBoxRect`/`copyMasksFromOriginal`。
