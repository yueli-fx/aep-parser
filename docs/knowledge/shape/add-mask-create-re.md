...
- `childTdmnPayload`（mutate_property_structural）的 pair 假设对 mask atom 不成立
  ...
  **MoveMask 同法落地（2026-06-12, Stable）**：`mutate_mask_move.go`——以各 mask mkif 定位全部三件套、
  校验连续，按 permutation 原指针重发 + 同步重排 scene/flat slice；双版本 ship-gate 2/2 PASS。
  **至此 mask 结构性 op 全收口（Add/Remove/Duplicate/Move 全 Stable）**——通用 Remove/Move/DuplicatePropertyGroup
  的 pair 假设对 mask 全部由专门 triple-aware 实现替代。
...
4. **lhd3 字段对 n≠4 顶点是错的（encodeBezier 遗留）**：mask 实测 `@0x14`=**4 恒定**
   ...
   **对的部分留下**：shape path 的 **AE 接受性** 确实不受影响——n=3 closed gate
   ...
   故无紧迫性。详 [path-keyframe-write-re](path-keyframe-write-re.md)。
...
- **PrintWindow 采集**：`Capture-WindowBitmap` 升级 `PrintWindow(PW_RENDERFULLCONTENT)`
  ...
  详 [ae-automation-occlusion-crashstate](../workflow/ae-automation-occlusion-crashstate.md)。
...
- ~~**RemoveMask**（triple-aware 删除）~~ **已 ship（2026-06-12, Stable）**——见上 §Atom 结构。
  ...
  tdbs 时间表，机制同 [path-keyframe-write-re](path-keyframe-write-re.md)）~~ **已 ship 2026-06-15（SetMaskPathKeyframes）** /
...
（`pages=(n+3)/4`，[lhd3-keyframe-capacity-pages](../keyframe-expression/lhd3-keyframe-capacity-pages.md)）此前只在 **shape path** 验到 6kf；
