...
- **Source-less**: a Camera/Light is defined entirely by its ldta + a
  ...
  (unlike Solid/Null/Adjustment, see [new-layer-types-scoping](new-layer-types-scoping.md)).
...
- **Embed-WHOLE-Layr, not from-scratch**: we clone an AE-native Camera/Light Layr
  ...
  needed extensive RE for ([multi-layer-silent-drop](multi-layer-silent-drop.md), [ae2020-shape-ldta-164-corrupt](../shape/ae2020-shape-ldta-164-corrupt.md)).
...
- **Property-based option setters — RESOLVED 2026-06-14 via parse-the-clone (for
  ...
  - **Camera Iris\*/Highlight\* — RESOLVED 2026-06-14 via synthesis-insert
    ...
    (`tmp_debug/gen_camera_iris.jsx` → historical `test_data/generated/fixtures/re_camera_iris.aep`), extract
    all 8 `(tdmn, LIST:tdbs)` pairs into current `internal/serializer/templates/options/camera_iris_leaves.bin`
  ...
  - **`ADBE Light Color` — RESOLVED 2026-06-14 via synthesis-insert (AE2020+2025
    ...
    historical `test_data/generated/fixtures/re_light_color.aep`), extract just the `(tdmn "ADBE Light Color",
    LIST:tdbs)` pair (`tmp_debug/extract_light_color_leaf` →
    current `internal/serializer/templates/options/light_color_leaf.bin`, a LIST(tdgp) wrapper), and have
...
- ~~NewTextLayer~~ — SHIPPED 2026-06-11 via the same embed-whole-Layr path
  ...
  see [text-btdk-length-variable-write-scoping](../text/text-btdk-length-variable-write-scoping.md)). ~~Solid/Null/Adjustment~~ —
  shipped 2026-06-10 ([new-layer-types-scoping](new-layer-types-scoping.md)). All layer kinds now creatable.
