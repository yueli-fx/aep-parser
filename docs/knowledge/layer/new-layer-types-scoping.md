...
Camera/Light shipped first ([camera-light-layer-create-re](camera-light-layer-create-re.md)); Solid/Null/
...
- The returned layer is already **parsed** (InsertLayer re-parses) — setters
  work immediately, no [add-effect-splice-re](../effects/add-effect-splice-re.md) Reopen needed.
- **ID-namespace trap (the one real reject)**: Go-built dest comps carry dummy
  ...
  NewComposition — details in [nextitemid-must-include-layer-ids](../parse-serialize/nextitemid-must-include-layer-ids.md).
...
bytes bypasses byte-level RE — see [v2-2-aelayer-structure](v2-2-aelayer-structure.md)) and the same
...
3. **AE-acceptance ship-gate is mandatory + high-risk** — footage-Item creation
   ...
   NewComposition 5 RE phases ([ae25-acceptance-gate](../workflow/ae25-acceptance-gate.md)). Budget for it.
