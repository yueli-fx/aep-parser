...
- error_type: —
- where: SetLayerTransform anchor encoding (internal/serializer) for AV-typed layers · surfaced in showcase/booyah-clone/gen_precomp1.go
...
[precomp-layer-source-id-re](precomp-layer-source-id-re.md) uses a bare precomp = template anchor, so it never exercised a
