...
**AE 唯一会保留的"未知数据"是它显式建模为可扩展的槽**:① 缺失插件的 effect 数据(无该插件的机器打开+保存仍保住 effect 字节);② 已知 property descriptor 内 AE 不认识的新版属性字节(见 [btdk-point-value-needs-formatpsreal](../text/btdk-point-value-needs-formatpsreal.md) 邻域 / 批7c 洞察:AE2020 opaque 保住 AE24+ enum)。这些是 AE 框架内的"剩余字节"保留,**不是任意顶层 chunk**。
