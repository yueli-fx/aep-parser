...
| vfx / showcase 生成器 | `showcase/<方向>/gen.go` | ✅ tracked |
...
原则:**有用→进 git(提到上面某个 tracked 家);没用→删;只 RE 一次但会改变未来行动的 findings 进 `docs/knowledge/<domain>/` 后删探针。** 不放进 `internal/aep`,避免污染 public API。
