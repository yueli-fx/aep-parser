...
"AddEffect 参数化" item ([add-effect-splice-re](add-effect-splice-re.md) finding 3 records the
...
- **A materialized param's value entry is small and self-contained** —
  ...
  [add-effect-splice-re](add-effect-splice-re.md) finding 5).
...
set time** — `aep.SetEffectParam(layer, fx, paramName, value)`
...
dichotomy ([add-mask-create-re](../shape/add-mask-create-re.md)) except source-less uses comp fractions,
...
`aep.AnimateEffectParam(layer, fx, paramName, []ScalarKeyframe)`——effects 最大真缺口：
...
`aep.AnimateEffectParamVec(layer, fx, paramName, []VectorKeyframe)`——把动画 effect param
...
`SetEffectParam`、`AnimateEffectParam` 和 `AnimateEffectParamVec` 仅接受参数名称，例如 `"Blurriness"` / `"模糊度"`；不接受原始 match-name。重名、未知名称在修改前报错。本文中的参数编号仍用于解释 AEP 字节布局；迁移和 Recipe 编译处理已解析的格式标识，直接使用内部 serializer 写入器。
