...
[property-indexed-group-structural-re](../property/property-indexed-group-structural-re.md)). The trailing `ADBE Group End` tdmn is
...
1. **Version-portable.** Templates are extracted from an **AE-2020-saved** file
   ...
   ([gradient-fill-write-re](../shape/gradient-fill-write-re.md)). One AE-2020 fixture serves both gate versions.
...
5. **tdpi = host-layer binding, AE validates it on open (the Phase-1 latent
   ...
   **2026-06-20 relocation:** the retarget now lives in `AddEffect`, NOT the
   ...
   [pseudo-layer-picker-tdpi-retarget-clobber](pseudo-layer-picker-tdpi-retarget-clobber.md) (incl. the 0-based-Index /
...
[effect-param-elision-synthesis-lite](effect-param-elision-synthesis-lite.md)); dual-version gated via the
...
- ~~**AddMask**~~ — SHIPPED 2026-06-11 (dual-version gated, from-scratch atom,
  ...
  `spliceEmptyParade`). See [add-mask-create-re](../shape/add-mask-create-re.md). NOTE the earlier "same
...
是 ALL-CAPS-带空格且大小写敏感**（同 chunk-id 大小写敏感 [chunk-id-case-tdb4](../parse-serialize/chunk-id-case-tdb4.md)）。probe11
