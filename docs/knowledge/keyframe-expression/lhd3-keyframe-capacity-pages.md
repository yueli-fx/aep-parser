...
- 2026-06-17 第三处验证（**mask path 动画 >4kf**，无代码改）：`encodePathTimeTable` 被 `makeMaskShapeOmSAnimated` 复用，但 **mask 严格性**（AE 急切解码 mask outline，容量字段错硬崩 0::42）下 >4kf 此前未单独 gate（`TestMGMaskPathKf` 只验 2kf=单页）。把它从 2kf bump 到 6kf（=2 页），AE 2020+2025 双版本 PASS（numKeys=6 读回不截断 + render 翻转 + resave 保留 6kf）→ 确认容量分页在 mask 上同样成立。详 [add-mask-create-re](../shape/add-mask-create-re.md) § 2026-06-17。
