...
# by the gates themselves); v2_2_* / ship_gate_1d_* come from tmp_debug/gen_*
# JSX. Anything left has NO known generator — regen impossible if lost.
$governed = [System.Collections.Generic.HashSet[string]]::new([StringComparer]::OrdinalIgnoreCase)
foreach ($j in $jobs) { foreach ($o in $j.Outputs) { [void]$governed.Add((Split-Path $o -Leaf)) } }
$trackedNames = git -C $repoRoot ls-files test_data | ForEach-Object { Split-Path $_ -Leaf }
$unknown = Get-ChildItem (Join-Path $repoRoot 'test_data') -Filter *.aep -File |
    Where-Object { -not $governed.Contains($_.Name) -and ($trackedNames -notcontains $_.Name) } |
    Select-Object -ExpandProperty Name
$gateByproducts = @($unknown | Where-Object { $_ -like 'ge_*' })
$tmpDebugGen = @($unknown | Where-Object { $_ -like 'v2_2_*' -or $_ -like 'ship_gate_1d_*' })
$ungoverned = @($unknown | Where-Object { $_ -notin $gateByproducts -and $_ -notin $tmpDebugGen })
...
if ($tmpDebugGen.Count -gt 0) {
    Write-Host "  tmp_debug-generated (v2_2_*/ship_gate_1d_*, generators in tmp_debug/gen_*.jsx): $($tmpDebugGen.Count)"
}
