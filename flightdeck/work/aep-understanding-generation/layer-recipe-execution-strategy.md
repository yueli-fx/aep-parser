...
| Effects | `effects[]` + static/expression/keyframed/layer-reference params | `AddEffect` / `SetEffectParam` / `SetEffectLayerParam` / `AnimateEffectParam` / `AnimateEffectParamVec` / `Property.SetExpression` | `layers[].effects[]` / `properties[].keyframes[]` / `properties[].layer_ref` | `expected_profile.effects[]` / `expected_profile.keyframes[]` | L3/L4 | static params + expressions + scalar/vector param keyframes + layer-reference params done |
...
`minimal-text-animator-*-value-keyframes.json` examples add
`value_keyframes` for scalar leaves (opacity/rotation/tracking/character
offset) and vector leaves (position/scale/fill color), then assert their leaf
properties through `expected_profile.keyframes[]`.
