...
| Effects | `effects[]` + static/expression/keyframed params | `AddEffect` / `SetEffectParam` / `AnimateEffectParam` / `AnimateEffectParamVec` / `Property.SetExpression` | `layers[].effects[]` / `properties[].keyframes[]` | `expected_profile.effects[]` / `expected_profile.keyframes[]` | L3/L4 | static params + expressions + scalar/vector param keyframes done |
