...
- Light layer recipes now support:
  - `light.falloff_start` -> `Layer.SetLightFalloffStart`
- Boundary: `light` options remain valid only on `type: "light"` layers. This
  slice adds Falloff Start; Falloff Start is only meaningful when
  `falloff_type` is not none/default. Falloff distance, cone, and other light
  options remain separate recipe slices.
- `examples/recipes/minimal-light-falloff-start.json` is a dedicated two-layer
  recipe: a `Light` layer with `falloff_type: 2` and `falloff_start: 100` plus
  a visible text layer. The embedded expected profile checks
  `ADBE Light Falloff Start = 100`.
- Verification:
  - `go test ./internal/recipe ./cmd/aeprecipe` passed.
  - `cmd/aeprecipe compile -recipe
    examples\recipes\minimal-light-falloff-start.json -out
    tmp_debug\recipes\minimal-light-falloff-start.aep -json` returned valid
    and all `profile_checks` passed, including
    `ADBE Light Falloff Start = 100`.
  - AE 2025 render oracle passed:
    - request:
      `tmp_debug\aeoracle\minimal_light_falloff_start\request.json`
    - done marker:
      `tmp_debug\aeoracle\minimal_light_falloff_start\aeoracle_render.done` =
      `ok`
    - metadata: status `ok`, AE `25.1x68`
    - PNG outputs:
      `f000000.png`, `f000015.png`, `f000030.png`
...
- Light layer recipes now support:
  - `light.falloff_distance` -> `Layer.SetLightFalloffDistance`
- Boundary: `light` options remain valid only on `type: "light"` layers. This
  slice completes the current falloff scalar trio (`falloff_type`,
  `falloff_start`, `falloff_distance`). Cone and other light options remain
  separate recipe slices.
- `examples/recipes/minimal-light-falloff-distance.json` is a dedicated
  two-layer recipe: a `Light` layer with `falloff_type: 2` and
  `falloff_distance: 750` plus a visible text layer. The embedded expected
  profile checks `ADBE Light Falloff Distance = 750`.
- Verification:
  - `go test ./internal/recipe ./cmd/aeprecipe` passed.
  - `cmd/aeprecipe compile -recipe
    examples\recipes\minimal-light-falloff-distance.json -out
    tmp_debug\recipes\minimal-light-falloff-distance.aep -json` returned valid
    and all `profile_checks` passed, including
    `ADBE Light Falloff Distance = 750`.
  - AE 2025 render oracle passed:
    - request:
      `tmp_debug\aeoracle\minimal_light_falloff_distance\request.json`
    - done marker:
      `tmp_debug\aeoracle\minimal_light_falloff_distance\aeoracle_render.done` =
      `ok`
    - metadata: status `ok`, AE `25.1x68`
    - PNG outputs:
      `f000000.png`, `f000015.png`, `f000030.png`
...
- Light layer recipes now support:
  - `light.cone_angle` -> `Layer.SetLightConeAngle`
- Boundary: `light` options remain valid only on `type: "light"` layers. This
  slice adds Cone Angle; it is only visually meaningful for spot lights. Cone
  feather and other light options remain separate recipe slices.
- `examples/recipes/minimal-light-cone-angle.json` is a dedicated two-layer
  recipe: a `Light` layer with `cone_angle: 72` plus a visible text layer. The
  embedded expected profile checks `ADBE Light Cone Angle = 72`.
- Verification:
  - `go test ./internal/recipe ./cmd/aeprecipe` passed.
  - `cmd/aeprecipe compile -recipe
    examples\recipes\minimal-light-cone-angle.json -out
    tmp_debug\recipes\minimal-light-cone-angle.aep -json` returned valid and
    all `profile_checks` passed, including `ADBE Light Cone Angle = 72`.
  - AE 2025 render oracle passed:
    - request:
      `tmp_debug\aeoracle\minimal_light_cone_angle\request.json`
    - done marker:
      `tmp_debug\aeoracle\minimal_light_cone_angle\aeoracle_render.done` =
      `ok`
    - metadata: status `ok`, AE `25.1x68`
    - PNG outputs:
      `f000000.png`, `f000015.png`, `f000030.png`
...
- Light layer recipes now support:
  - `light.cone_feather` -> `Layer.SetLightConeFeather`
- Boundary: `light` options remain valid only on `type: "light"` layers. This
  slice adds Cone Feather; it is only visually meaningful for spot lights.
  Other light options remain separate recipe slices.
- `examples/recipes/minimal-light-cone-feather.json` is a dedicated two-layer
  recipe: a `Light` layer with `cone_feather: 35` plus a visible text layer. The
  embedded expected profile checks `ADBE Light Cone Feather 2 = 35`.
- Verification:
  - `go test ./internal/recipe ./cmd/aeprecipe` passed.
  - `cmd/aeprecipe compile -recipe
    examples\recipes\minimal-light-cone-feather.json -out
    tmp_debug\recipes\minimal-light-cone-feather.aep -json` returned valid and
    all `profile_checks` passed, including `ADBE Light Cone Feather 2 = 35`.
  - AE 2025 render oracle passed:
    - request:
      `tmp_debug\aeoracle\minimal_light_cone_feather\request.json`
    - done marker:
      `tmp_debug\aeoracle\minimal_light_cone_feather\aeoracle_render.done` =
      `ok`
    - metadata: status `ok`, AE `25.1x68`
    - PNG outputs:
      `f000000.png`, `f000015.png`, `f000030.png`
...
- Shape recipes now support:
  - `shape.fill_composite_order` -> `FillNode.SetCompositeOrder`
- Boundary: `fill_composite_order` is a string enum with `above_previous` and
  `below_previous`. Validation rejects other values before compile.
- `examples/recipes/minimal-shape-fill-composite-order.json` is a dedicated
  one-layer shape recipe with a rect fill set to `below_previous`. The embedded
  expected profile checks `ADBE Vector Composite Order = 2`.
- Verification:
  - RED was observed with `go test ./internal/recipe`: compiled profile still
    read `ADBE Vector Composite Order = 1`, and capability
    `FillNode.SetCompositeOrder` was absent.
  - `go test ./internal/recipe ./cmd/aeprecipe` passed.
  - `cmd/aeprecipe compile -recipe
    examples\recipes\minimal-shape-fill-composite-order.json -out
    tmp_debug\recipes\minimal-shape-fill-composite-order.aep -json` returned
    valid, reported `FillNode.SetCompositeOrder`, and all embedded
    `profile_checks` passed.
  - AE 2025 render oracle passed:
    - request:
      `tmp_debug\aeoracle\minimal_shape_fill_composite_order\request.json`
    - done marker:
      `tmp_debug\aeoracle\minimal_shape_fill_composite_order\aeoracle_render.done`
      = `ok`
    - metadata: status `ok`, AE `25.1x68`
    - PNG outputs:
      `f000000.png`, `f000015.png`, `f000030.png`
...
- Shape gradient stroke recipes now support:
  - `shape.gradient_stroke.highlight_length` ->
    `GradientStrokeNode.SetHighlightLength`
  - `shape.gradient_stroke.highlight_angle` ->
    `GradientStrokeNode.SetHighlightAngle`
- Boundary: `highlight_length` is a radial highlight offset percentage and
  must be between `-100` and `100`; `highlight_angle` is a scalar angle in
  degrees.
- `examples/recipes/minimal-shape-gradient-stroke-highlight.json` is a
  dedicated one-layer radial gradient stroke recipe with highlight length `65`
  and angle `40`. The embedded expected profile checks
  `ADBE Vector Grad HiLite Length` and `ADBE Vector Grad HiLite Angle`.
- Verification:
  - RED was observed with `go test ./internal/recipe`: compiled profile still
    read `ADBE Vector Grad HiLite Length = 0`, highlight capabilities were
    absent, and invalid highlight length was not refused.
  - `go test ./internal/recipe ./cmd/aeprecipe` passed.
  - `cmd/aeprecipe compile -recipe
    examples\recipes\minimal-shape-gradient-stroke-highlight.json -out
    tmp_debug\recipes\minimal-shape-gradient-stroke-highlight.aep -json`
    returned valid, reported `GradientStrokeNode.SetHighlightLength` and
    `GradientStrokeNode.SetHighlightAngle`, and all embedded `profile_checks`
    passed.
  - AE 2025 render oracle passed:
    - request:
      `tmp_debug\aeoracle\minimal_shape_gradient_stroke_highlight\request.json`
    - done marker:
      `tmp_debug\aeoracle\minimal_shape_gradient_stroke_highlight\aeoracle_render.done`
      = `ok`
    - metadata: status `ok`, AE `25.1x68`
    - PNG outputs:
      `f000000.png`, `f000015.png`, `f000030.png`
...
- Shape gradient stroke recipes now support:
  - `shape.gradient_stroke.line_cap` -> `GradientStrokeNode.SetLineCap`
  - `shape.gradient_stroke.line_join` -> `GradientStrokeNode.SetLineJoin`
  - `shape.gradient_stroke.miter_limit` -> `GradientStrokeNode.SetMiterLimit`
- Boundary: `line_cap` is `butt`, `round`, or `projecting`; `line_join` is
  `miter`, `round`, or `bevel`; `miter_limit` must be at least `1`.
- `examples/recipes/minimal-shape-gradient-stroke-style.json` is a dedicated
  one-layer linear gradient stroke recipe with projecting caps, bevel joins,
  and miter limit `9`. The embedded expected profile checks
  `ADBE Vector Stroke Line Cap`, `ADBE Vector Stroke Line Join`, and
  `ADBE Vector Stroke Miter Limit`.
- Verification:
  - RED was observed with `go test ./internal/recipe`: compiled profile still
    read `ADBE Vector Stroke Line Cap = 2`, style capabilities were absent,
    and invalid style inputs were not refused.
  - `go test ./internal/recipe ./cmd/aeprecipe` passed.
  - `cmd/aeprecipe compile -recipe
    examples\recipes\minimal-shape-gradient-stroke-style.json -out
    tmp_debug\recipes\minimal-shape-gradient-stroke-style.aep -json` returned
    valid, reported `GradientStrokeNode.SetLineCap`,
    `GradientStrokeNode.SetLineJoin`, and `GradientStrokeNode.SetMiterLimit`,
    and all embedded `profile_checks` passed.
  - AE 2025 render oracle passed:
    - request:
      `tmp_debug\aeoracle\minimal_shape_gradient_stroke_style\request.json`
    - done marker:
      `tmp_debug\aeoracle\minimal_shape_gradient_stroke_style\aeoracle_render.done`
      = `ok`
    - metadata: status `ok`, AE `25.1x68`
    - PNG outputs:
      `f000000.png`, `f000015.png`, `f000030.png`
