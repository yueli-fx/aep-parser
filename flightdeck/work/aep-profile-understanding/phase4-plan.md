...
- [x] Add `internal/gapledger/gap_test.go`.
- [x] Test diff `ActionWrite` maps to `write-gap`.
- [x] Test diff `ActionSemantics` maps to `semantic-gap`.
- [x] Test diff `ActionPlugin` maps to `plugin-gap`.
- [x] Test diff `ActionParse` maps to `parse-gap`.
- [x] Test diff `ActionInvestigate` maps to `investigate-gap`.
- [x] Assert each gap has deterministic ID, source project, observed target, evidence, severity, action type, profile path, and human notes.
- [x] Implement `internal/gapledger/gap.go`.
- [x] Run `go test ./internal/gapledger`.

### Task 2: RED/GREEN Render Gap Mapping

- [x] Add tests for `FromRenderCompare`.
- [x] When `DifferentPixels == 0`, expect no gaps.
- [x] When `DifferentPixels > 0`, emit one `render-gap` with metric fields in `details`.
- [x] Include `expected_path`, `actual_path`, total pixels, different pixels, percent, max channel delta, and threshold.
- [x] Run `go test ./internal/gapledger`.

### Task 3: CLI Diff Gaps

- [x] Create `cmd/aepgaps/main.go`.
- [x] Implement `diff`:
...
- [x] Text output should show count plus first 50 `id type severity action profile_path`.
- [x] JSON output should include schema version, source project, observed target, and gaps.
- [x] Verify with identical fixture and different fixture pair.
...
- [x] Verify with generated 2x2 PNGs.

### Task 5: Verification and Commit

- [x] Run `go test ./internal/gapledger ./cmd/aepgaps`.
- [x] Run `go test ./...`.
- [x] Run `go vet ./...`.
- [x] Run `git diff --check`.
- [x] Run reserved-word scan on touched files.
- [x] Run `go run ./cmd/aepgaps diff -json flightdeck/showcase/text/text.aep flightdeck/showcase/text/text.aep`.
- [x] Run `go run ./cmd/aepgaps diff flightdeck/showcase/text/text.aep flightdeck/showcase/effects/effects.aep` and confirm exit code 1.
- [x] Run `go run ./cmd/aepgaps render` against generated test PNGs and confirm exit code 1.
- [x] Update Flightdeck status.
- [x] Commit with `feat(gapledger): add structured gap reports`.
