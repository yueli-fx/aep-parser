---
status: active
when_to_read: 想理解库怎么造形状/效果(为什么必须基于 .bin);看到 v2_2/v22 命名困惑;纠结 effect/shape 跨 AE 版本要不要分版本号;评估从零生成 vs embed-body
applies_to: [embed-template, bin-template, parse-the-clone, silent-drop, v2-2-naming, milestone, version-portability, per-version-template, ship-gate, effect, shape, architecture, opaque-preservation]
last_updated: 2026-06-17
when_to_update: embed-template 模板布局重组(2026-06-17 已分子目录),或写策略从 embed-body 转向从零生成,或 per-version 模板机制落地
---

# Embedded-template architecture, v2_2 naming & version portability

> 三个会话调查的合并总结(2026-06-17,用户提问驱动)。**暂存待后续处理**——可能 graduate-合并进 CLAUDE.md 架构段或保留为 reference。相关:`specs/per-version-template-versioning.md`(响应方案)、`incidents/v2-2-aelayer-structure.md`(silent-drop 教训)。
...
**为什么这么设计**:AE 是**严格校验器**——chunk 格式有大量未公开/版本相关/默认即省略(elided)字段,任何字节结构跟它 canonical 输出不一致的,要么判损坏、要么静默丢(V2.2 血泪,详 `incidents/v2-2-aelayer-structure.md` / `multi-layer-silent-drop.md`)。逐字节逆出来从零生成既不可行也脆。「拿真实例克隆 + 只改看懂的值槽」是双版本 ship-gate 反复验证的安全路径。
...
- `v2_2` 是开发阶段:**V2.1 地基 → V2.2 从零创建图层/形状层 → V3 结构性 mutation**(见 `archive/specs/2026-05-22-v2-{1,2}-*` / `v3-*`)。
- `incidents/v2-2-aelayer-structure.md` 标题即 *"V2.2 ship gate FAIL — Layr 结构…"*。
...
**要不要加版本号?——现在不加(YAGNI),触发条件明确**:某模板的某版本 gate 真红了才加,且分级响应(局部→条件 CODE;整份→版本键化 map + 覆盖文件)。详 `specs/per-version-template-versioning.md`。**双版本 gate 就是你想要的版本分裂探测器。**
