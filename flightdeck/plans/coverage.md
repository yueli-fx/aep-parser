...
- `RenderQueueItem.{Comp（via comp_id@0x08→Composition）, Status（raw u32@0x0C）, Name（template_name@0x5A win-1252）, Comment（RCom→Utf8）, TimeSpanStart/Duration（按 time_span_source@2148 解析：LENGTH_OF_COMP/WORK_AREA/CUSTOM）, OutputModules, NumOutputModules()}`
- `OutputModule.{Name（Als2 后 Utf8[0]）, FileTemplate（Als2 后 Utf8[1]）, FullPath（alas JSON fullpath）}`
- JSON 导出 `render_queue` 节点（snake_case 自有 schema）
...
- **defer 后续 slice**：全 RenderSettings enum（26）/ OutputModule 128B settings / format options×7 / 任何写 / `file` 模板变量解析。详 `plans/2026-06-01-py-aep-p3-renderqueue-reader-plan.md`
