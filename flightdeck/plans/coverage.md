...
- **常用矢量滤镜家族完整收齐（10 件）**：Trim/Repeater/RoundCorners/Offset/Merge/ZigZag/PuckerBloat/Twist/WigglePaths——只剩 Wiggle Transform（`ADBE Vector Filter - Wiggler`，带嵌套 Transform 组同 Repeater）。详 `incidents/trim-paths-vector-filter-re.md` § 复用确认 — Wiggle Paths + 家族小结。
