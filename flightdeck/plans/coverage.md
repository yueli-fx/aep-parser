...
- `(g *VectorGroup) AddZigZag()` → `ZigZagNode`（`SetSize`/`SetDetail` + `Size()`/`Detail()` getter；Size=振幅 px 默认 5、Detail=ridges/段 默认 10）— ✅ **AE 2020+2025 双版本渲染像素 ship-gate PASS**（`TestMGZigZag_AEShipGate_AE20{20,25}`：400×400 Rect + Size=40/Detail=8 → 四边尖齿 starburst；逐列扫顶白 y spread=78（峰 301 谷 379=±40 围绕原边 340）、center 白、Size/Detail resave 读回；眼验）。
- **磁盘形态**（套 vein 第 7 次，distort 型）：`ADBE Vector Filter - Zigzag` body = `ADBE Vector Zigzag Size` + `ADBE Vector Zigzag Detail` 双 1D f64 BE @cdat[0:8] → `lowerShapeScalar`（含 animated）。`Zigzag Points`（enum 默认 elide）未建模。模板 `v2_2_shape_zigzag_body.bin`（708B/7 children，`gen_shape_zigzag.jsx`）。stack [Rect, Fill, ZigZag]。
...
- `(g *VectorGroup) AddTwist()` → `TwistNode`（`SetAngle` + `Angle()` getter；度数，`NewTwistNode` 默认 Angle=0=identity，正/负旋；Vec2 Twist Center 默认 [0,0] elide 未建模）— ✅ **AE 2020+2025 双版本渲染像素 ship-gate PASS**（`TestMGTwist_AEShipGate_AE20{20,25}`：400×400 白 Rect + Angle=150 → 风车螺旋；center 钉白、四原始角清空 4/4、mirror-asym=44（twist 手性独有签名，对称方块=假绿）、Angle resave 读回；眼验 silhouette）。
- **磁盘形态**（套 vein 第 9 次，distort 型，与 Round Corners 完全同构）：`ADBE Vector Filter - Twist` body = headline 子流 `ADBE Vector Twist Angle`（1D f64 BE @cdat[0:8] 度数）→ `lowerShapeScalar`（含 animated flip）。无嵌套组、无 enum、无 elision（body 402B/5 children）。模板 `v2_2_shape_twist_body.bin`（`gen_shape_twist.jsx`）。stack [Rect, Fill, Twist]。
