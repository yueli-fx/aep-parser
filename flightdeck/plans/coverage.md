...
- **Property dimensions separation R (P3, py-aep parity)**: `DimensionsSeparated()`（tdsb byte 3 `_enable_flags` bit 1）+ `IsSeparationLeader() / IsSeparationFollower() / SeparationDimension()`（纯 match-name：leader=="ADBE Position"，follower=="ADBE Position_0/1/2"→dim 0/1/2，非 follower 返回 -1）。fixture `transform_separated.aep`。W（分离/合并 toggle）结构性，需 ship-gate，deferred。⚠ 发现既有 parser bug：3D 层 Transform Group 解析截断（Orientation 起 desync，Position_2/Scale/Opacity 丢），见 `incidents/transform-group-3d-truncation.md`。
