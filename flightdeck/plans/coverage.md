...
- **Property dimensions separation W (P3 §3C, Alpha)**: `Property.SetDimensionsSeparated(bool)` — 结构性 separate↔merge 双向 toggle，静态 Position（2D + 3D），**双版本 ship-gate 6/6 PASS**（AE 2020 + AE 2025：3D sep / merge / 2D sep）。Go 输出 chunk 树 byte-structural 等同 AE 自存。字节机制：separate = leader 翻 tdsb byte2→0x08+bit1、值重置默认 `[w/2,h/2,0]`、真值迁入 Position_0/1（3D 再合成 `Position_2`，靠 `layer.Is3D` 区分非 Components）；merge = leader 取回 `[X,Y,Z]`、删全部 follower（AE resave 会重新预分配 zeroed Pos0/1 → merged 两种等价表示）。fallible 的 Position_2 re-parse 在所有就地改动前 → 失败即无副作用。**仅 animated Position 暂搁**（keyframe 流迁移未 RE）。详 `incidents/separate-dimensions-write-mechanics.md`。注：3D 层 Transform Group 解出的属性比 py-aep 少不是 bug —— AE 省略未改的默认属性（py-aep 合成 schema，我们不合成 `Elided`），见 `incidents/transform-group-default-omission.md`。
...
- `RenderQueue.RemoveItem(index)` **W**（结构性，**双版本 ship-gate PASS**）：删 item 联动四处 —— LItm 移除 `[RCom?]+list+'LOm '`、settings ldat splice 2246B 块 + lhd3 count -1、Rout per-item block 删 + header 比例减。settings ldat 跨 item 共享 → 删后重挂存活 item 的 settingsBlock 别名。Go 输出 LRdr 子树 byte-structural 等同 AE `item.remove()`；AE 2020+2025 读回 numItems-1 + 存活 comp 正确。剩 **RQ item 增删的 add/insert**（需从零合成 2246B 设置块或克隆，deferred）。详 `incidents/render-queue-delete-mechanics.md`
