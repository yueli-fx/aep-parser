...
- **`Footage.SetSolidColor(rgb)` / `SetSolidSize(w, h)` W（2026-06-12, Alpha）**：solid 独立 setter——opti @0x0A ARGB f32 + sspc @0x20/0x24 u16 length-preserving 补丁，与 NewSolidLayer 双 gate 同字节路径（standalone 未单独 gate）。非 solid / 越界（色值 0..1、尺寸 1..30000）refuse；round-trip 测试 ×3。收掉 backlog「Solid 系小件」第一项；剩余：导入 solid footage 落 dest 根而非 Solids folder（AE 接受，整理性差异，需求驱动）。
