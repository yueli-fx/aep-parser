...
- **Essential Graphics 模板名 W (2026-06-12, Alpha)**: `Composition.SetMotionGraphicsTemplateName` — length-variable 改写三代 `CIFO/CIF2/CIF3 × CpS2+CapS` 共 6 个 Utf8 槽（AE 三代面板内容 byte-identical，漏写会版本性显示旧名）。空名 refuse；Go-built comp（embed 模板自带三代空壳）同样可写。round-trip 测试 ×2；ship-gate 待 AddEssentialProperty 搭车后 Alpha→Stable。EG 全量 W（创建 controller + OvG2/CPrp 绑定）RE 已完成、实现进行中：`plans/2026-06-12-essential-graphics-write.md`。
