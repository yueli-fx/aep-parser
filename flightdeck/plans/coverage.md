...
- **AddEffect W (2026-06-10, Alpha)**: `aep.AddEffect(layer, effectMatchName) (*Effect, error)` + `aep.SupportedEffects() []string` — 给 layer 的 `ADBE Effect Parade` 追加一个内置效果。**AE 2020 + AE 2025 双版本 ship-gate 2/2 PASS**（`TestAddEffect_AEShipGate_AE20{20,25}`：baseline 3 effect + Go 加第 4 个 → AE 接受不损坏、回读 4 effect 顺序正确、AE resave 保留）。字节机制：parade `LIST(tdgp)` 内 effect = `(tdmn[40B], LIST:sspc)` pair，以 `ADBE Group End` tdmn 哨兵收尾；AddEffect 在哨兵前 splice 一对（**同 `DuplicatePropertyGroup` 的 pair-splice 机制，但 sspc payload 来自嵌入 AE-native 模板**而非克隆兄弟）。LIST size 由 `rifx.Chunk.Write` bottom-up 自动重算（同 gradient/SetPath length-variable 路径）。返回的 `*Effect` 经 spliced pair 二次 `collectEffects` 解析 → 可即时调参（`Property.SetStaticValue` 对 effect 参数生效，已 ship）。原子（snapshot parade chunk + scene children + flat Effects + warnings → 任何 parser warning 即回滚）。**Phase-1**：要求 layer 已有 Effect Parade group（AE-parsed AV layer 都有）；NewShapeLayer 的 from-scratch 层无 parade → refuse（parade auto-create = Phase 2 deferred）。**效果模板限于已嵌入集**（当前仅 `ADBE Gaussian Blur 2`，extracted from `re_property_struct_baseline.aep`；版本可移植——AE-2020 字节 AE 2025 接受，同 gradient 发现）。模板 `internal/serializer/templates/effect_gaussian_blur_body.bin`。
