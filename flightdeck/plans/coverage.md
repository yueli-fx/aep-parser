...
- **AddEffect W (2026-06-10, Alpha)**: `aep.AddEffect(layer, effectMatchName) (*Effect, error)` + `aep.SupportedEffects() []string` — 给 layer 的 `ADBE Effect Parade` 追加一个内置效果，返回解析好的 `*Effect`（可即时调参——`Property.SetStaticValue` 对 effect 参数生效，已 ship）。**AE 2020 + AE 2025 双版本 ship-gate 10/10 PASS**（`TestAddEffect_AEShipGate_AE20{20,25}` 子测，5 效果 × 2 版本：Gaussian Blur / Fill / Tint / Easy Levels2 / Pro Levels2，按 sspc payload 体量取样 1.7KB→20.6KB；AE 接受不损坏、回读 4 effect 顺序正确、AE resave 保留）。字节机制：parade `LIST(tdgp)` 内 effect = `(tdmn[40B], LIST:sspc)` pair，以 `ADBE Group End` tdmn 哨兵收尾；AddEffect 在哨兵前 splice 一对（**同 `DuplicatePropertyGroup` 的 pair-splice 机制，但 sspc payload 来自嵌入 AE-native 模板**而非克隆兄弟）。LIST size 由 `rifx.Chunk.Write` bottom-up 自动重算（同 gradient/SetPath length-variable 路径）。原子（snapshot parade chunk + scene children + flat Effects + warnings → 任何 parser warning 即回滚）。
  - **效果库 = 12 内置效果**（match-name → 嵌入模板 `internal/serializer/templates/effect_adbe_*.bin`）：Gaussian Blur 2 / Fill / Tint / Brightness & Contrast 2 / Tritone / Easy Levels2 / Pro Levels2 / HUE SATURATION / Box Blur / Glo2 / Invert / Exposure2。全部从单一 AE-2020-saved fixture `re_effect_library.aep`（`re_effect_library.jsx` 应用 curated 集合）extract；版本可移植——AE-2020 字节 AE 2025 接受（同 gradient 发现）。**12 个全部 Go round-trip 验证**（`TestAddEffect_AllTemplates_RoundTrip`：splice+WriteAEP+re-parse），其中 5 个取样 AE 双版本 ship-gate；其余 7 个走同一机制未单独 gate。
  - **配套 API**：`aep.RemoveEffect(layer, index)`（AddEffect 的逆，delegate 到 ship-gate-green `RemovePropertyGroup`，index 校验，Go round-trip 验证）+ 12 个 `aep.Effect*` typed match-name 常量（`EffectGaussianBlur`…`EffectExposure`，registry 单源 + `TestEffectConstants_MatchRegistry` 防漂移）。
  - **Phase-1 限制**：要求 layer 已有 Effect Parade group（AE-parsed AV layer 都有）；`NewShapeLayer` 的 from-scratch 层无 parade → refuse。**Phase 2 deferred（已 RE，非快赢，详 `incidents/add-effect-splice-re.md`）**：from-scratch shape 层加效果撞「write-time re-lowering 墙」（`syncShapeLayerChunks` 重新 lower 脏 shape 层，chunk 编辑被丢）+ scene 禁持 rifx chunk → 需通用 effect-param lowering 或 serializer-side pending-effects；今有 write+reopen 变通路径。另：含 layer/path-reference 参数的效果（需 sspc id remap）+ 扩库。
