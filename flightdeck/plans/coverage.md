...
- **Property dimensions separation W (P3 §3C, Alpha)**: `Property.SetDimensionsSeparated(true)` — 结构性 merge→separate toggle，**双版本 ship-gate PASS**（AE 2020 + AE 2025：dimensionsSeparated=true + Position_0/1/2 读回迁移值）。Go 输出 chunk 树 byte-structural 等同 AE 自存（`re_separate_dims_{before,after}.aep` RE 对账）。字节机制：leader 翻 tdsb byte2→0x08 + byte3 bit1、值重置为默认 `[w/2,h/2,0]`；真值迁入 Position_0/1/2（各清自身 bit1）；3D 的 Z follower `Position_2` 合成（克隆 Position_1 的 tdmn+tdbs）。fallible 的 Position_2 re-parse 在所有就地改动前完成 → 失败即无副作用。scope：**仅静态 3D Position 的 separate 方向**；merge（separate→merged）/ 2D / animated 暂搁（各需独立 RE + ship-gate）。详 `incidents/separate-dimensions-write-mechanics.md`。注：3D 层 Transform Group 解出的属性比 py-aep 少不是 bug —— AE 省略未改的默认属性（py-aep 合成 schema，我们不合成 `Elided`），见 `incidents/transform-group-default-omission.md`。
