...
- ✅ **批8b/8c**(shape 几何):**11 个** 升 ae-accept。SetDirection(rect/ell,洁癖洞 shape_enums)·新 gate `TestShapeGeom` 覆盖 Rect Roundness/Position·Star Inner/Outer Roundness/Position·Repeater Offset/Rotation·Trim Start/Offset(Go-reparse streamCdat 按 match-name 验值存活,1D+2D)。残 shape 3:SetAnchor(Repeater/Wiggler)/SetScale(Wiggler)——~~Repeater Anchor match-name 实测错需 RE,Wiggler transform generic match-name 歧义,defer~~ ✅ **批23 收口**(probe dump 模板坐实 match-name 本就对,旧"实测错"是验证侧坑;扩 TestShapeGeom 双版本验)。**shape 域 roundtrip 清零**。
...
- ✅ **批19**(comp,e818475):**DuplicateComposition** 升 ae-accept(双版本)。此前只人工 JSX-gated(coverage.md 手记)、无自动 Go _AEShipGate test → 封顶 roundtrip;无版本特性 → 双版本可达。新 gate `TestCompDuplicate_AEShipGate_AE2020/AE2025` + verify_comp_duplicate.jsx:from-scratch ORIG(solids A,B;B parent→A)→ DuplicateComposition("DUP")。AE DOM 验 distinct CompItem(id 不同)+ 两 comp layers=[A,B](深拷贝)+ **DUP 的 B.parent 指向 DUP 自己的 A(intra-comp ref remap,非源 comp)** + layer source 共享(id 相等,solid 非复制)+ comp 设置克隆(w/h/fps/dur)。comp roundtrip 6→5。
- ✅ **批20**(project,796af6b):**9 工程设置 setter** 升 ae-accept(双版本)。**验证洁癖洞**:这批早被 project-settings showcase(verify.jsx,2026-06-14 RE 修过 nnhd→legacy nhed 显示组)在 AE2020+2025 DOM readback 实测一致,但无自动 Go _AEShipGate test → 封顶 roundtrip。新 gate `TestProjectSettings_AEShipGate_AE2020/AE2025` + verify_project_settings.jsx:from-scratch project + 单 shape 层,每项设 NON-DEFAULT 值后 app.project DOM readback。SetBitsPerChannel(16)/SetTimeDisplayType(Frames)/SetFramesCountType(Start1)/SetFramesUseFeetFrames(true)/SetFeetFramesFilmType(MM16)/SetFootageTimecodeDisplayStartType(UseSourceMedia)/SetLinearBlending(true)/SetExpressionEngine(javascript-1.0,expr 域)+ **SetDisplayStartFrame 传递性升**(与 SetFramesCountType 写同一 nnhd byte20,gate 验 displayStartFrame DOM=1)。project roundtrip 19→11,expr 2→1。**另**(490ae38):**SetLinearizeWorkingSpace boundary 记 DOM 假绿负结论** —— incident project-flag-chunks-lnrb-lnrp 实锤 lnrp 字节写对、AE 接受文件,但 app.project.linearizeWorkingSpace 即便 AE 自存文件 reload 也读 false(OCIO/CMS derived state,不绑 lnrp chunk)→ DOM 不可值验、封顶 acceptance,留 roundtrip。SetWorkingGamma(dwga)同理弃(showcase 当年排除,AE2025 色彩管理工作空间下大概率读不回,单 cap 低 ROI)。
- ✅ **批21**(comp idta RE,3ca3839 + fb7687e):**comp SetComment + SetLabel** 升 ae-accept(双版本,gate `comp_idta`,纯 AE DOM readback)。用户选方向①(comp idta RE)。RE = AE2020 原生 fixture `re_comp_idta.aep`(各设一项 comment/label/draft3d + BARE)diff:**① item has-comment flag = idta @0x39**(comment 时 00→01,类比 layer ldta @0x3C);**② cmta 位置 = 紧跟 Utf8、非末尾**(AE Item LIST:idta Utf8 cmta dats cdta…)。**修真 bug**:旧 `setItemComment` append cmta 到 Item LIST 末尾 + 没设 idta flag → Go round-trip 绿但 **AE 打不开文件**(弹"打开项目"框)——layer-setcomment incident "Item LIST 同坑"预言成真,且比 layer 多一个位置坑(layer 末尾 AE 容忍、item 不容忍)。修:cmta 插到 Utf8 之后 + 设 idta @0x39(comp+footage 共用 setItemComment;footage 载体未单独验留 roundtrip)。codec 加 `IdtaHasComment=0x39`。**SetDraft3D 抓出 false-green**:cdta @0x8A bit0 字节与 AE 原生 DRAFT 逐位一致,AE reopen 读 comp.draft3d=false(derived 态,incident `comp-setdraft3d-false-green`,同 lnrp/timeRemap 家族),留 roundtrip。**过程踩坑**:gate 的 resave 步骤弹 AE Save 框(规则无 Save-As 处理)+ 反复 force-kill 污染 AE safe-mode 态 → 改纯 DOM readback(去 resave)+ `clear_ae_crashstate.ps1` 清状态后跑通。comp roundtrip 5→3。
- ✅ **批22**(footage idta,0165b5b):**footage SetComment + SetLabel** 升 ae-accept(双版本,gate `footage_idta`,纯 DOM readback)。批21 的 `setItemComment`/`setItemLabel` 修复(item cmta 位置 + idta @0x39 flag)comp+footage 共用,但 footage 载体批21 没单独 AE 验留 roundtrip。本批补:新 gate `TestFootageIdta_AEShipGate_AE2020/AE2025` + verify_footage_idta.jsx。**载体 = 单 from-scratch solid footage**(`NewSolidLayer`→opti "Soli",Reopen 接 idta/cmta/itemList back-ref)。**关键勘探(Go 探针 probe_footage_idta)**:solid 的 **Item-level Utf8 为空**(名字活在 opti chunk @0x1A),cmta 正确插在该空 Utf8 之后(非末尾,位置对);故 **jsx 按 `instanceof FootageItem` 取唯一 solid、不按名**(避 from-scratch 名假绿,红线7)。双绿:AE2025 12.8s / AE2020 115.3s,footage.comment + footage.label DOM 全对(附带 footage name=FTGSOLID 也存活)。**过程**:首跑撞 cold-start splash「正在初始化本地化设置」exit-2(前台还挂着游戏)——按既有 ritual `clear_ae_crashstate.ps1` warm-up + comp_idta 已知-good 对照(13s 绿)确认环境,再 retry 即双过。**operator context 入 incident `ae-automation-occlusion-crashstate`**(用户不在本机,exit-2 别甩用户;verify jsx 禁 resave)。
- ✅ **批23**(shape transform,ff02637,**shape 域收口**):**Repeater/Wiggler transform anchor/scale 3 残项**(RepeaterTransform.SetAnchor · WigglerTransform.SetAnchor/SetScale)升 ae-accept。**勘探推翻旧假说**:Go 探针(`tmp_debug/probe_shape_xform_names`)dump 嵌入模板 repeater_body.bin / wiggletransform_body.bin 的真实 tdmn 流名,坐实 `lower_shape_node.go` 用的 match-name(`ADBE Vector Repeater Anchor` / `ADBE Vector Wiggler Anchor` / `ADBE Vector Wiggler Scale`)与 template **逐字一致**——cockpit/旧 plan 记的"Repeater Anchor match-name 实测错 / Wiggler 歧义需 RE"是**早期验证侧坑、非字节 bug**(字节本就对、Go round-trip 自洽,probe_shape_xform_rt 实证 3 值 round-trip OK)。残因仅是无 AE gate。**扩展既有 `TestShapeGeom_AEShipGate_AE2020/AE2025`**(resave-preservation 2D 值验,与 batch-8b 同机制,verify_shape_geom.jsx 纯 open+resave 显式路径、无需改):repeater 加 SetAnchor([15,25]) + 新 WiggleTransform 节点 SetAnchor([30,40])/SetScale([120,80]),Go 侧 streamCdat 验 3 个 match-name。双绿 AE2025 17.1s/AE2020 22.5s。
- `ae-accept` 35→**217**;`roundtrip` 279→**97**(io 剩 2=WriteAEP/WriteJSON N/A;**shape 域 roundtrip 清零**)。
- **project-DOM from-scratch 簇收尾**:9 升 + 1 传递,其余 project 残 11 = 无 DOM(WorkingGamma 假绿风险/AudioSampleRate/Gpu 机器相关/TimecodeBase/TranspGrid)· lnrp 已记假绿 · CMS 4(AE24+ 需既有 CMS chunk,AE2020 开不了→不可达双版本)· SetPath(需 footage 载体)。
...
- ~~**shape**(批8 后残 ~14)~~ ✅ **域收口**(批8b/8c + 批23 transform anchor/scale,roundtrip 清零):
