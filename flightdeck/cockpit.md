...
3. gradient 余项：✅ **radial type** + ✅ **HiLite 高光** + ✅ **G-Stroke 方向/type** 均已通（2026-06-14；G-Fill `SetGradientType`/`SetHighlight*`、G-Stroke `SetStartPoint`/`SetEndPoint`/`SetGradientType`/`SetHighlight*` 全 parity；HiLite Angle 0=+X；模板 G-Fill 15-child·G-Stroke 29-child；`TestMGGradientHilite_*`/`TestMGGradStrokeGeom_*` 双版本像素 PASS，详 `incidents/gradient-fill-write-re.md`）。余：**G-Stroke HiLite 独立像素 gate**（边际，机制已被 G-Fill 证）· type/direction/highlight **read-back**（hydrate write-only）· **stroke geometry**（Width/Cap/Join/Dashes/Taper/Wave，烤 18px）。**gradient showcase 已扩 3×3**（加 06_RadialHiLite + 第三排 G-Stroke 3 格，agent 已自验 png）→ **`待review`，待用户真机复核**后翻 complete。
