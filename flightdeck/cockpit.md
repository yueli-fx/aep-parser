...
**Active focus**: **roundtrip→ae-accept 补验 arc**(2026-06-17 起;详 `## 下一步` + `plans/2026-06-17-roundtrip-ae-accept-backfill.md`)。库能力主线早已全收口,此 arc 把"标 stable 但只 Go round-trip、从没让真 AE 消化"的写能力按域补双版本 AE 验(验证洁癖洞)。已推进至批7(ae-accept 35→**113**,roundtrip→201):layer-set 主体(批1-3)+ camera/light options 17(批4)+ 8 classic material(批5a)+ 22 Composition setter(批6)+ **13 SetRun* text style(批7,单 run whole-doc DOM)**。含 RE 修好 SetComment 假绿 · 批6 勘出 SetFrameRate+SetDuration 真 bug(incident)· 批7 修 SetRunTsume FormatPSReal bug。**不变量**:知识单一家 = flightdeck + CLAUDE.md(auto-memory 已退役);能力真相源 = capindex(`go run ./cmd/capindex -q <词>`,CI 强制零漏标);每渲染类双版本 AE ship-gate(红线4)。火焰=番外(`incidents/procedural-fx-over-vector.md`)。
...
- **批7 ✅**(13 SetRun* style 双版本,修 tsume bug)。残 text(→7b):SetRun* enum/index(FontIndex/DigitSet/AutoKernType/BaselineOption/LineJoinType/NoBreak,可仿批7 扩 gate)· **SetParagraph* 10(⚠ 5 个 indent/spacing 疑同 tsume FormatPSReal bug,先验;段落 DOM 需 paragraphRange/AE2022+)** · ManualKerning/AddFont/animator。
- **下一批 = 7b(SetRun enum 扩 gate,最快)或 shape 23**(渲染像素红线4)→ keyframe/mask → marker(SetChapter/URL/CuePoint 归错 comp domain)→ project/meta → render-queue 40(多 acceptance)。layer-set bool 残 + 6b(comp idta) 按需。
