...
- **aep-understanding-generation** (`work/aep-understanding-generation/index.md`) — 新方向 spec：把 `aepdissect -json` / `Project.WriteJSON` / Booyah 渲染脚本 / 能力表收敛成 profile → diff → render oracle → gap ledger → recipe generation 的长期流水线。Phase 1 已抽 `internal/profile`，`aepdissect -json` 已切到 stable profile builder；自审补齐 track matte refs + parsed in/out points（2051e04）。Phase 2 已实现 `internal/profilediff` / `cmd/aepdiff` / JSON ignore rules（adfb7da）。Phase 3 已实现 `internal/aeoracle` / `cmd/aeoracle` / generic JSX render harness（053b3fc）。当前进入 Phase 4 gap ledger。
...
- **aep-understanding-generation Phase 4**：按 `work/aep-understanding-generation/phase4-plan.md` 实现 `internal/gapledger` / `cmd/aepgaps`，把 diff/render observations 变成结构化 gap；暂不进入生成器。
