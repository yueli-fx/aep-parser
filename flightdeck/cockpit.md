...
- **aep-understanding-generation Phase 6 recipe IR**：执行 `work/aep-understanding-generation/phase6-recipe-ir-plan.md`。第一版只做 one comp + solid/text/shape + transform 的最小 recipe compiler；所有 unsupported constructs 必须显式 refusal/downgrade。不要开 automated correction loops。
