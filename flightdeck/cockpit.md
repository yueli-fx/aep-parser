...
**Last updated**: 2026-06-17 by claude（本会话连推 roundtrip→ae-accept 补验 arc 批11+批12：**keyframe 域 12 mutate setter**(SetIn/OutInterp·In/OutTemporalEase·In/OutSpatialTangent·SetTime/Value/FrameTime·SetStaticValue·Insert/DeleteKeyframe,新 gate keyframe_mutate,spatial tangent 首试即 DOM 读回,残 SetLockedRatio 1 defer)·**structural 域 8 op 全收口**(Delete/Duplicate/Insert/MoveLayer + 4 Move 封装,新自动 gate structural_ops 取代旧人工 JSX-gate,载体须 solid/AV)。均双版本 AE gate。ae-accept 164→**184**,roundtrip 150→**130**。陷阱:2D layer Position 解析为 3D(z=0);Delete/Dup/Insert 拒非 AV 层。）

**Active focus**: **roundtrip→ae-accept 补验 arc**(2026-06-17 起;详 `## 下一步` + `plans/2026-06-17-roundtrip-ae-accept-backfill.md`)。库能力主线早已全收口,此 arc 把"标 stable 但只 Go round-trip、从没让真 AE 消化"的写能力按域补双版本 AE 验(验证洁癖洞)。已推进至批12(ae-accept 35→**184**,roundtrip→**130**):layer-set 主体(批1-3)+ camera/light 17(批4)+ classic material 8(批5a)+ Composition 22(批6)+ text 27/33(批7/7b/7c)+ shape 20(批8/8b/8c)+ marker 9(批9)+ mask 选项 8(批10)+ **keyframe 12 mutate(批11)+ structural 8 op 全收口(批12)**。修 3 真 bug(SetComment 假绿 · SetFrameRate+Duration incident · btdk FormatPSReal incident:tsume+5 段落)。两通用洞察:**AE2020 opaque 保留不识别的 AE24+ 属性**(批7c)· **既有 render gate 常 set+DOM 读回 roundtrip-tag 的 setter = 验证洁癖洞**(批4/8,自跑确认绿后补标)。**不变量**:知识单一家 = flightdeck + CLAUDE.md(auto-memory 已退役);能力真相源 = capindex(`go run ./cmd/capindex -q <词>`,CI 强制零漏标);每渲染类双版本 AE ship-gate(红线4)。火焰=番外(`incidents/procedural-fx-over-vector.md`)。
...
- **roundtrip 剩 130,域分布**:render-queue 40(多无 ScriptingAPI,acceptance 顶,期望低)· layer-set 39(残:material advanced/light·source/trackmatte/alternate 需多层载体·bool 残 Stretch/AutoOrient/IsNull/audio/frameblend·6b comp idta)· project 19(多 header/flag 读不回→acceptance)· meta 10(**read/helper,ae-accept 本不适用,非目标**)· comp 6(idta RE + 域标可疑项)· text 6(硬尾)· io 4 · shape 3(RE)· expr 2(closed)· keyframe 1(defer)。
- **下一批候选**(剩多为硬骨头/低 ROI):layer-set bool 残(需 video/precomp/两层载体)· text-font(SetRunFontIndex+AddFont,plan 标 text 首选)· comp(SetOrientation/SetPosition 先核实是否 camera mis-domain=洁癖洞,SetComment/Label 需 idta RE)· project/render-queue(多 acceptance 封顶)。
