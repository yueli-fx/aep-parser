...
- **aep-understanding-generation** (`work/aep-understanding-generation/index.md`) — 新方向 spec：把 `aepdissect -json` / `Project.WriteJSON` / Booyah 渲染脚本 / 能力表收敛成 profile → diff → render oracle → gap ledger → recipe generation 的长期流水线。Phase 1 已抽 `internal/profile`，`aepdissect -json` 已切到 stable profile builder；自审补齐 track matte refs + parsed in/out points（2051e04）。Phase 2 已实现 `internal/profilediff` / `cmd/aepdiff` / JSON ignore rules（adfb7da）。Phase 3 已实现 `internal/aeoracle` / `cmd/aeoracle` / generic JSX render harness（053b3fc）。Phase 4 已实现 `internal/gapledger` / `cmd/aepgaps`；Phase 5 已实现 `internal/sliceworkflow` / `cmd/aepslices`，fixture smoke 通过。Phase 6 前仍需真实外部 AEP 验收。
...
- **aep-understanding-generation 外部 AEP 验收**：用 `cmd/aepslices` 跑一个真实非 Booyah 工程，确认 profile → diff → render oracle → gap ledger 输出不依赖 bespoke project code；暂不进入生成器。
