...
**Active focus**: **roundtrip→ae-accept 补验 arc**(2026-06-17 起;详 `## 下一步` + `plans/2026-06-17-roundtrip-ae-accept-backfill.md`)。库能力主线早已全收口,此 arc 把"标 stable 但只 Go round-trip、从没让真 AE 消化"的写能力按域补双版本 AE 验(验证洁癖洞)。已推进至批6(ae-accept 35→**100**,roundtrip→214):layer-set 主体(批1-3)+ camera/light options 17(批4,补标已覆盖洞)+ 8 classic material(批5a,author AE2020-native 载体)+ **22 Composition setter(批6,from-scratch gate)**。含 RE 修好 SetComment 假绿 · 批6 勘出 SetFrameRate+SetDuration 真 bug(新 incident)。**不变量**:知识单一家 = flightdeck + CLAUDE.md(auto-memory 已退役);能力真相源 = capindex(`go run ./cmd/capindex -q <词>`,CI 强制零漏标);每渲染类双版本 AE ship-gate(红线4)。火焰=番外(`incidents/procedural-fx-over-vector.md`)。
...
- **下一批 = text 33**(btdk DOM 可读 font/size/justification/tracking,部分已有 text gate,核实重复)→ shape 23(渲染像素,红线4)→ keyframe/mask → marker(SetChapter/URL/CuePoint…归错 comp domain)→ project/meta → render-queue 40(多数仅 acceptance)。layer-set bool 残项(Stretch/AutoOrient/IsNull/audio/frameblend)+ 6b(comp idta) 按需。
