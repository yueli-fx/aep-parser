...
- **aep-understanding-generation 下一步**：recipe IR 已能 one comp + comp background_color/motion_blur/work_area/renderer/resolution_factor/pixel_aspect/display_start_time/frame_blending + solid/text/shape + transform + supported built-in effect + static effect params + shape stroke/detail/style/dashes/taper/wave/polystar + Trim Paths/Round Corners/Offset Paths/ZigZag/Pucker & Bloat/Twist/Wiggle Paths/Wiggle Transform/Repeater/Merge Paths + profile-visible text style + Position/Anchor/Scale/Rotation/Opacity keyframe profile checks 编译 AEP，且 report 会列 used capabilities / downgrades / refusals / profile_checks。下一刀继续补更多 comp settings、shape filters、表达式或 transform keyframe/ease（以 writer 支持为边界）；更多 text style 只补 profile 可读字段。不要开 automated correction loops。
