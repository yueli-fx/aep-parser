...
- **roundtrip 剩 104,域分布(便宜双版本 DOM 验已真正榨干)**:render-queue 40(acceptance 顶)· layer-set 25(material-advanced 8/3D-geometry 3=不可达双版本· trackmatte-explicit 4=单版本验· 2 false-green-alpha=SetStretch/SetTimeRemapEnabled· audio/frameblend 需音视频源· alternate-source 2/light-source AE24+ niche· markers-locked 无 DOM)· project 11(WorkingGamma/AudioSampleRate/Gpu/TimecodeBase/TranspGrid 无 DOM 或假绿· **lnrp 已记假绿**· CMS 4=AE24+ 需既有 CMS chunk·SetPath 需 footage 载体)· meta 10(read/helper N/A)· comp 5(idta RE:SetComment/Label/Draft3D · Guide SetPosition/Orientation 无 DOM)· text 4(animator+kerning 硬尾)· io 4 · shape 3(RE)· expr 1(closed)· keyframe 1(defer)。
- **下一批候选(已无便宜双版本 DOM)**:① comp idta RE(SetComment/Label item-level + SetDraft3D,需 AE 原生 set→diff idta 找 has-comment flag)· ② shape 3 match-name RE(Repeater/Wiggler Anchor/Scale)· ③ 需音视频载体(audio/frameblend,造素材)· ④ render-queue/project header 走 acceptance-preservation(量大、tier 低于 ae-accept)。再推任一方向都比批1-20 贵,**择一深入,非顺手 drain**。
