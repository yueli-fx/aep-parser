...
**➡ 优先级5 表达式〔下一步〕**：roadmap spec 优先级5。已知坑：`SetExpression` 栽在「Go round-trip 绿但 AE 不求值」（`incidents/expression-enable-byte-pair.md`——tdb4 @0x77/@0x78 has-expression-marker + expressionEnabled）。从该 incident 起步，先 RE 表达式启用字节对再做渲染类 ship-gate（红线1：Go round-trip ≠ AE 接受）。
