...
**Last updated**: 2026-06-15 by claude（Text Animators 扩 Position + Scale：`AddTextPositionAnimator`〔slide-in，质心 Δ=260px〕+ `AddTextScaleAnimator`〔shrink-in，ink 面积 1070→313〕双版本渲染 gate PASS。机制泛化验证〔Scale 零改动只抽模板+facade〕→ Rotation 照搬 scalar 即免费，Fill Color 待 color cdat RE。前：Opacity 动画器 + offset 扫光首落地。）
...
**按需 backlog（非阻塞，需求驱动）**：
- 文字：~~Opacity~~ ✓ + ~~Position~~ ✓ + ~~Scale~~ ✓（双版本 gate PASS）；剩 Rotation（scalar，照搬 Opacity 免费）· Fill Color（color cdat 待 RE）· animate leaf 本身（关键帧驱动 Position/Scale 值）· Range Advanced(Mode/Shape/Smoothness)· 多 Selector · Wiggly/Expression Selector · text-animator structural op（Remove/Dup/Move）补 ship-gate（现 Alpha）。详 `incidents/text-animator-create-re.md`
