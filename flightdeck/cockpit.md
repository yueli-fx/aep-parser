...
- **aep-understanding-generation 下一步**：recipe IR 已能 one comp + solid/text/shape + transform 编译 AEP；下一刀应把 `cmd/aeprecipe` 的临时静态 capability map 升级为复用 `cmd/capindex` 背后的机器可读能力索引，并在 recipe report 里记录 used capabilities / downgrades / refusals。不要开 automated correction loops。
