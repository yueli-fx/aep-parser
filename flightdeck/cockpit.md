...
**Last updated**: 2026-06-17 by claude（本会话连推补验 arc 批11-15 + chunk-probe 实验：**批11 keyframe 12 mutate**·**批12 structural 8 op 全收口**·**批13 layer-set transform/time 7**(SetPosition/Anchor/Scale/Opacity + SetFrame{In,Out}Point/StartTime)·**批14 track-matte**(classic SetTrackMatte 双版本;explicit 4 AE2025 单版本验、双版本不可达留 roundtrip)·**批15 layer-set flag**(SetAutoOrient + SetCollapseTransform 双版本;**SetStretch 抓出确认 false-green**——写 stretch 分数 AE 按 in/out span 重算回 100,降 stable→alpha,incident 记)。ae-accept 164→**194**,roundtrip 150→**120**。**另**:chunk-probe 实验坐实「AE 保存丢弃任意未知 RIFX chunk(root+item,双版本)、打开不损坏」→ incident `ae-drops-unknown-chunks-on-resave`(元数据走 native comment / XMP-未支持)。陷阱:2D layer Position 解析为 3D(z=0)·transform 静态 setter 需 materialized 载体·SetInPoint=source-relative(AE=startTime+)·explicit matte TargetAE2025 fingerprint 被 AE2024 拒。**⚠ 待用户决策**:explicit-matte 4 是否破例 mint 单版本 ae-accept。）

**Active focus**: **roundtrip→ae-accept 补验 arc**(2026-06-17 起;详 `## 下一步` + `plans/2026-06-17-roundtrip-ae-accept-backfill.md`)。库能力主线早已全收口,此 arc 把"标 stable 但只 Go round-trip、从没让真 AE 消化"的写能力按域补双版本 AE 验(验证洁癖洞)。已推进至批15(ae-accept 35→**194**,roundtrip→**120**):layer-set 主体(批1-3)+ camera/light 17(批4)+ classic material 8(批5a)+ Composition 22(批6)+ text 27/33(批7/7b/7c)+ shape 20(批8/8b/8c)+ marker 9(批9)+ mask 选项 8(批10)+ keyframe 12 mutate(批11)+ structural 8 op 全收口(批12)+ layer-set transform/time 7(批13)+ track-matte classic 双版本(批14;explicit 4 单版本留 roundtrip)+ **layer-set AutoOrient/CollapseTransform(批15;SetStretch 抓出 false-green 降 alpha)**。修 3 真 bug(SetComment 假绿 · SetFrameRate+Duration incident · btdk FormatPSReal incident:tsume+5 段落)。两通用洞察:**AE2020 opaque 保留不识别的 AE24+ 属性**(批7c)· **既有 render gate 常 set+DOM 读回 roundtrip-tag 的 setter = 验证洁癖洞**(批4/8,自跑确认绿后补标)。**不变量**:知识单一家 = flightdeck + CLAUDE.md(auto-memory 已退役);能力真相源 = capindex(`go run ./cmd/capindex -q <词>`,CI 强制零漏标);每渲染类双版本 AE ship-gate(红线4)。火焰=番外(`incidents/procedural-fx-over-vector.md`)。
...
- **roundtrip 剩 120,域分布**:render-queue 40(多无 ScriptingAPI,acceptance 顶,期望低)· layer-set 26(残:material advanced 8/3D-geometry 3=不可达双版本(AE2020 无 ray-traced/Advanced-3D)· source/alternate/light-source 需特殊载体· trackmatte explicit 4=单版本已验· SetStretch=false-green-alpha · bool 残 IsNull/audio/frameblend/effects/timeremap 需载体· markers-locked 无 DOM=acceptance)· project 19(多 header/flag→acceptance)· meta 10(**read/helper,ae-accept 不适用,非目标**)· comp 6(idta RE)· text 6(硬尾)· io 4 · shape 3(RE)· expr 2(closed)· keyframe 1(defer)。
