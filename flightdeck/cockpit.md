...
**Last updated**: 2026-06-12 by claude（**shape path open/closed parser bug 修复 + encodeBezier 二次纠错**：dump AE-native fixture `v2_2_shape_path_re.aep`（含开放 path，层名 tan_path/tri_closed/tri_open 即 ground truth）纠正上一轮——AE-native shape path 的 closed/lhd3 字段值与 mask **完全一致**（shph[3] open=0x09 / @0x14=4 恒 / @0x18=1 恒 / @0x0C·@0x1C 疑 nextPow2 容量），encodeBezier 写的是与 mask 同源的**通用偏差非 mask 独有**，只是 shape 侧 AE 容忍（接受+读回顶点对，n=3 gate 证）。**顺带逮到并修了确证的公共 API parser bug**：`decodeShapePath` 用恒 0x01 的 shph[0x14] 读 closed → 开放 shape path 误报 Closed=true（影响 `Layer.ShapePaths`+JSON），改读 shph[3]（同 hydrate），fixture 三 path closed=false/true/false 全对 + 回归 `TestShapePathOpenIsNotClosed`，全量 vet/test 绿。writer byte-faithfulness（shph[3] open=0x09 等）+ 开放 shape path ship-gate = follow-up。前一更新 = **SetEffectParam 控件类型补齐**：泛型 per-control-type 模板从 3 类扩到全 8 类（+angle/color/2D/3D/slider，touch-all fixture 一次提取）+ Point3D Control 扩库 #30，双版本 gate 13 项全 PASS（含 cross-effect color/angle pard-patch）。RE：point 参数 cdat = 层坐标空间分数（有源=源尺寸、source-less=comp 尺寸、z 除 height）；color cdat = ARGB×255。⚠ 坑：ExtendScript `""+colorValue` 拼接抛「除以零」——首轮 gate 假 reject，bisect 整文件才发现是 verify JSX 日志行炸了（陷阱已记 re-fixture.md）。详 `incidents/effect-param-elision-synthesis-lite.md`）
...
📌 **新 follow-up（writer byte-faithfulness，需 AE ship-gate）**：让 encodeBezier 直接写 AE-native 值（shph[3] open=0x09 / @0x14=4 / @0x18=1 确定；@0x0C/@0x1C 待 n=4..8 RE 确证 nextPow2 容量假设），shape/mask 写路径即 byte-faithful、mask patch 可化简；**关键缺口：开放 shape path 的 AE 接受性从无 ship-gate（现有 shape gate 全 closed）**。
