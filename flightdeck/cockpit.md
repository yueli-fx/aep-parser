...
- **aep-understanding-generation** (`work/aep-understanding-generation/index.md`) — 新方向 spec：把 `aepdissect -json` / `Project.WriteJSON` / Booyah 渲染脚本 / 能力表收敛成 profile → diff → render oracle → gap ledger → recipe generation 的长期流水线。Phase 0 审计产物已写：导出面审计 / coverage matrix / profile contract；人工审核通过后再进 Phase 1 `internal/profile`。
...
- **aep-understanding-generation Phase 0 人工审核点**：确认 `phase0-audit.md` / `profile-coverage.md` / `profile-contract.md` 后，再进入 Phase 1 `internal/profile` 抽取；暂不进入生成器。
