...
- **aep-understanding-generation 下一步**：recipe IR 已能 one comp + solid/text/shape + transform + supported built-in effect + static effect params + shape stroke/detail + profile-visible text style + keyframe profile checks 编译 AEP，且 report 会列 used capabilities / downgrades / refusals / profile_checks。下一刀继续补更多 transform keyframe channel、表达式或 shape filters；更多 text style 只补 profile 可读字段。不要开 automated correction loops。
