...
- **aep-understanding-generation** (`work/aep-understanding-generation/index.md`) — 新方向 spec：把 `aepdissect -json` / `Project.WriteJSON` / Booyah 渲染脚本 / 能力表收敛成 profile → diff → render oracle → gap ledger → recipe generation 的长期流水线。下一步先做实现计划，推荐 Phase 1 `internal/profile`。
...
- **aep-understanding-generation Phase 1 plan**：抽 `cmd/aepdissect` JSON profile 到 `internal/profile`，建立稳定 schema/path/golden tests；暂不进入生成器。
