...
- **aep-understanding-generation** (`work/aep-understanding-generation/index.md`) — 长期流水线：profile → diff → render oracle → gap ledger → slice workflow → recipe IR → technique facts/portrait/report → versioned AEP migration。当前恢复点是 versioned AEP migration：`cmd/aepmigrate assess/convert` 已有保守转换路径，convert 现已覆盖 supported non-shape layer metadata/switches/refs、dynamic layer transforms、single-run text style、solid-backed null-controller、camera/light options/light-source refs、supported built-in effects、effect layer-ref params、effect param expressions、scalar keyframes、vector keyframes；writer target 现覆盖 AE2020-AE2025，text baseline / text animator / text style / static effects / auto-orient / layer mask / shape(all current recipes) / project / comp / precomp / camera / light / structural representatives / dynamic transform / dynamic effect-param 均已有 PD-6x6 writer evidence，layer 全家族 6x6 边界为 576 total / 536 pass / 10 blocked / 0 failed / 30 skipped（仅 matte contract），最新无 AE 全量矩阵为 846 total / 840 pass / 0 blocked / 0 failed / 6 skipped，`cmd/aepmigrate matrix` 支持 `-ae-versions` 将 writer target 与 AE2020-AE2025 打开验证 host 维度分离，并带 AE-open case 上限保护；继续前先读 topic index、history 最新段和当前 diff。
