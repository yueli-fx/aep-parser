...
- **shape 次要子属性**（Fill/Stroke Opacity·BlendMode·CompositeOrder、Shape Direction；部分 runtime-only 逐个甄别）← 推荐下一项。
