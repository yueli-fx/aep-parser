...
- **aep-understanding-generation Phase 6**：执行 `work/aep-understanding-generation/phase6-plan.md`。第一刀补 `aeoracle clone-request` + `aeoracle compare-set`，让 Booyah 原工程和 `flightdeck/showcase/booyah-clone/booyah-clone.aep` 可用同一 sentinel frame set 渲染并批量 PNG diff；recipe IR 和 correction loops 必须等该 render compare report 存在后再开。
