...
- **aep-understanding-generation 下一步**：recipe IR 已能 one comp + solid/text/shape + transform + supported built-in effect 编译 AEP，且 report 会列 used capabilities / downgrades / refusals。下一刀选一个窄覆盖面：effect 参数、更多 shape/text 控制，或 recipe→profile expected-output contract。不要开 automated correction loops。
