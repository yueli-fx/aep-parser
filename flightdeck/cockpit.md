...
- **aep-understanding-generation** (`work/aep-understanding-generation/index.md`) — 新方向 spec：把 `aepdissect -json` / `Project.WriteJSON` / Booyah 渲染脚本 / 能力表收敛成 profile → diff → render oracle → gap ledger → recipe generation 的长期流水线。已吸收外部 review：先做 Phase 0 导出面审计 + evidence/path/profile 契约锁定，再进入 Phase 1 `internal/profile`。
...
- **aep-understanding-generation Phase 0 plan**：审计 `cmd/aepdissect -json` vs `Project.WriteJSON`，产出 coverage matrix、字段准入、evidence ladder、stable path contract；之后才抽 `internal/profile`，暂不进入生成器。
