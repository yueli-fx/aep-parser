...
- **roundtrip 剩 114,域分布(便宜双版本 DOM 验已榨干)**:render-queue 40(acceptance 顶)· layer-set 22(残:material-advanced 8/3D-geometry 3=不可达双版本· trackmatte-explicit 4=单版本验· **2 false-green-alpha=SetStretch/SetTimeRemapEnabled**· audio/frameblend 需音视频源· alternate-source 2/light-source AE24+ niche· markers-locked 无 DOM)· project 19(header/flag→acceptance)· meta 10(read/helper N/A)· comp 6(idta RE)· text 4(animator+kerning 硬尾)· io 4 · shape 3(RE)· expr 2(closed)· keyframe 1(defer)。
- **下一批候选**:剩多为 acceptance 封顶(render-queue/project)· RE 活(comp idta SetComment/Label · shape match-name)· 需音视频载体(audio/frameblend)。**便宜双版本 DOM 段落收尾**;再推需造音视频素材或 RE。
