...
1. **AddEffect 参数化**：per-effect typed param helper（今为 raw `SetStaticValue` by match-name）。⚠ **设计墙（incident finding 3）**：模板 = 默认实例，大多数参数 default-elided 无 chunk 可写——typed helper 有用的前提是先解决 (a) 参数 synthesis（backlog 大件）或 (b) 换提取策略（fixture 里逐参数 touch 非默认值再 extract，但模板会带非默认值需 post-patch 回默认）。先做设计决策再动。（✅ 扩库 wave-2 已落，库 = 29 全部双版本 gated，2026-06-11；再扩库 = 需求驱动，⚠ 带 layer/path 引用参数的效果其 tdpi 指向非宿主层，需选择性 remap——盲 retarget-all 会写坏，详 incident finding 5）
