...
- **aep-understanding-generation 下一步**：recipe IR 已能 one comp + solid/text/shape + transform + supported built-in effect + static effect params 编译 AEP，且 report 会列 used capabilities / downgrades / refusals / profile_checks。下一刀继续补更多 shape/text 控制、text style、表达式或关键帧能力。不要开 automated correction loops。
