...
- **versioned-aep-migration** (`work/versioned-aep-migration/index.md`) — active mainline：AE2020-AE2025 migration matrix、coverage/current JSON、domain batch inventory、host-open planning、registry checkpoint。连续 package queue 已跑到无下一包；继续前读 `goal.md` / `current.md`，下一步是最终 closure gate 和一次性提交。
...
- **versioned AEP migration**：连续 package queue 已无 candidate；运行最终 registry/checkpoint/full-test closure gate，确认后按用户要求只做一次最终提交。不要默认跑全量 `-ae-open`，除非显式扩大/关闭 `-max-ae-open-cases`。
