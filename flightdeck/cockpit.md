...
- **aep-understanding-generation 下一步**：recipe IR 已能 one comp + solid/text/shape + transform + supported built-in effect + static effect params 编译 AEP，且 report 会列 used capabilities / downgrades / refusals。下一刀优先做 recipe→profile expected-output contract，或继续补更多 shape/text 控制。不要开 automated correction loops。
