...
1. **AddEffect 参数化后续扩展**（✅ synthesis-lite 已 ship 2026-06-11：`aep.SetEffectParam` 双版本 gated，pilot = GB 3 参数；详 `incidents/effect-param-elision-synthesis-lite.md`）。剩余可推：(a) 参数模板扩库——per-effect touch-all fixture + `extract_effect_params`（⚠ 跨版本默认值漂移 finding 2）；(b) **泛型 per-control-type 模板试验**（~7 个模板覆盖全库，需 gate 证 tdb4 非参数特定——成立则免去逐效果提取）。（✅ 效果库 = 29 全部双版本 gated；再扩库 = 需求驱动，⚠ 引用参数效果 tdpi 指向非宿主层详 add-effect incident finding 5）
