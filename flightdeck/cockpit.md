...
Focus: `codebase-hygiene-survey` 已完成第二批审计：2 条 stale-codepath knowledge 已修/重分类，26 个 generator delete-candidate 已人工拆为 keep-active 6、keep-provenance-gap 5、archive/delete-candidate 15、direct-delete-now 0，internal 大包 watch 已确认不在本 survey 内拆包。下一步是修 generator provenance-gap 并决定 15 个候选的产物/基线处置策略。Booyah 复刻仍待用户真机验收；technique research 暂停在 glitch recipe/showcase gate 后。
...
- **codebase-hygiene-survey** (`work/codebase-hygiene-survey/index.md`) — active cleanup survey：四张初始 ledger 已生成；knowledge header repair + README sync + tmp safe-delete batch 已完成并通过 gates；2 条 stale-codepath knowledge 已修；26 个 generator delete-candidate 已细分为 keep/provenance-gap/archive-delete；internal watch 已复核。下一步修 5 个 generator provenance-gap，并决定 15 个 archive/delete candidate 的 fixture/baseline 策略。

## Next

- **Codebase hygiene survey**：修 `work/codebase-hygiene-survey/ledgers/generator-provenance-review.md` 里的 5 个 `keep-provenance-gap`，再决定 15 个 `archive-or-delete-candidate` 是否连同 tracked fixture / split-roundtrip baseline hash 一起删、迁移为 durable evidence，或保留为历史基线。
