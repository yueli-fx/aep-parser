...
- ~~**Offset Copies**~~ ✅ **2026-06-15**（synthesis-insert 首发矢量滤镜；蓝本见下）。剩 Offset Line Join / Miter / Copy Offset 同 splice 路径，按需。
- **Trim Type**（Simultaneously/Individually，elide 无 slot → synthesis-insert，**蓝本已现成** `spliceShapeLeafBeforeGroupEnd`，同 `SetMaterialOption` 路径）← 推荐下一项。
...
**蓝本（synthesis-insert 推广到矢量滤镜，2026-06-15 验透）**：clone elided leaf 模板 → `spliceShapeLeafBeforeGroupEnd`（GroupEnd 前插 (tdmn,tdbs) pair）→ 覆写 cdat，仅当值≠默认。不污染默认 body、不需 hydration（filter 靠 opaque chunk 穿越 Reopen）。详 `trim-paths-vector-filter-re.md` § Offset Copies。
