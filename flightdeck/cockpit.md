...
**Active focus**: **roundtrip→ae-accept 补验 arc**(2026-06-17 起;详 `## 下一步` + `plans/2026-06-17-roundtrip-ae-accept-backfill.md`)。库能力主线早已全收口,此 arc 把"标 stable 但只 Go round-trip、从没让真 AE 消化"的写能力按域补双版本 AE 验(验证洁癖洞)。已推进至批10(ae-accept 35→**164**,roundtrip→**150**):layer-set 主体(批1-3)+ camera/light 17(批4)+ classic material 8(批5a)+ Composition 22(批6)+ text 27/33(批7/7b/7c)+ shape 20(批8/8b/8c)+ **marker 9(批9)+ mask 选项 8(批10)**。修 3 真 bug(SetComment 假绿 · SetFrameRate+Duration incident · btdk FormatPSReal incident:tsume+5 段落)。两通用洞察:**AE2020 opaque 保留不识别的 AE24+ 属性**(批7c)· **既有 render gate 常 set+DOM 读回 roundtrip-tag 的 setter = 验证洁癖洞**(批4/8,自跑确认绿后补标)。**不变量**:知识单一家 = flightdeck + CLAUDE.md(auto-memory 已退役);能力真相源 = capindex(`go run ./cmd/capindex -q <词>`,CI 强制零漏标);每渲染类双版本 AE ship-gate(红线4)。火焰=番外(`incidents/procedural-fx-over-vector.md`)。
...
- **roundtrip 剩 150,域分布**:render-queue 40(多无 ScriptingAPI,acceptance 顶,期望低)· layer-set 39(残:material advanced/light·source/trackmatte/alternate 需多层载体·bool 残 Stretch/AutoOrient/IsNull/audio/frameblend·6b comp idta)· project 19(多 header/flag 读不回→acceptance)· keyframe 13(interp/tangent/ease,查 mg_ease 洁癖洞)· meta 10 · structural 8 · comp 6 · text 6(硬尾)· io 4 · shape 3 · expr 2(closed)。
- **下一批候选**:keyframe(查 mg_ease/mg_*_kf 洁癖洞)· layer-set bool 残(需载体)· text-font · structural · project/meta(部分 acceptance)。
