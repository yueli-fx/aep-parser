---
status: active
when_to_read: setting a non-zero Anchor Point on a precomp / footage / AV layer via SetLayerTransform; a precomp/footage layer renders off-screen or blank while a shape/text layer with the same anchor renders fine; AE DOM reads a layer's anchor as ~value×source-dimension (e.g. 960 → 1843200); deciding anchor units for SetLayerTransform on different layer kinds
applies_to: [setlayertransform, layertransform, anchor-point, anchor, av-layer, precomp, footage, fraction-vs-pixel, source-dimension, render-offscreen, blank-render, red-line-4, false-green, roundtrip-not-ae, booyah, comp5, library-bug]
