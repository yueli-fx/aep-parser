---
status: active
when_to_read: 想往 .aep 嵌入自定义数据/作者元数据/版权/任意 stash;纠结自定义 RIFX chunk 会不会被 AE 保留;评估 lib-write→lib-read vs AE-resave 的数据存活;选元数据通道(comment/marker/XMP/自定义chunk)
applies_to: [custom-chunk, metadata, author, xmp, opaque-preservation, resave, rifx, negative-finding, comment, marker-comment, ae-dom-rebuild, data-stash]
