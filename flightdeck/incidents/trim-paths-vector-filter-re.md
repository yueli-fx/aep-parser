...
**家族小结（蓝本 8 次全绿）**：Trim · Repeater(+嵌套 Transform 组) · RoundCorners · Offset · Merge(combine·fill 在上) · ZigZag · Pucker&Bloat。三步蓝本（probe→抽 body→cdat 覆写）对所有常用矢量滤镜成立；唯二变量 = ① 子流集合/elision 边界（先 all-non-default fixture 逼 AE 不 elide）② **combine 型 fill 位置反**（Merge 需 fill 在 stack 顶）。剩 Twist/Wiggle 等同 vein 预期可推。
