...
SUMMARY: 历史 Go 文件仍有大量 gofmt diff；CI 只对新的公开/安全产品面做格式门禁，全仓格式化必须作为独立机械重写处理。
READ WHEN: changing CI formatting checks, running `gofmt -w .`, or planning repository-wide formatting cleanup
RECHECK WHEN: 全仓 Go 文件完成一次独立 gofmt baseline commit 后
