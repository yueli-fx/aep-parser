---
status: active
when_to_read: 要给新 AE 版本(2027/2028…)生成效果参数字典;想知道某 matchName/参数槽的人类语义或默认值;实现工具消费字典(翻译参数名、按 default 识别用户调过的参数);AE 切语言后脚本写文件报 Permission denied;dump 脚本输出 0KB
applies_to: [effects-dict, dump, matchname, default-value, recipe-signal, ae-version, language-switch, scripting-permission, application-xml, savePrefAsString, lut-dialog, aepdissect, cross-language-join]
