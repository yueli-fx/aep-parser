...
| ④ | カクッ | 🔶**待review**(双版本 AE-accept + DOM 对账 PASS) | `gen_kakuh.go`:2 shape 层(「シェイプレイヤー 3」over「2」),各=**stroked rect(1635×810,无 fill,描边 0.8px)+ Trim Paths(Start=97/Offset=5.3)** + 层级 `SetLayerTransform` Scale(2kf 64%→100%)/Opacity(20kf flicker),L0 加 RotateZ=180。Go round-trip 逐值对账 ✓(rect/stroke-width/trim-start-offset/scale-kf/op-kf/rotZ 全等)。**AE2020≡AE2025 接受**:4 comp 全在、④ 2 层不 drop、2×Stroke+2×Trim 被 AE 吃进、dump OK。**关键修正**:`SetLayerTransform.Scale()` 单位=**percent**(非 parser 的 fraction),scale 需 ×100(同 Opacity 约定,初版 0.64 写成 0.64%→64% 修正)。**结构/保真 delta**(render-neutral,记账):①from-scratch shape 多套一层 `Vector Group→Vectors Group`(同 comp ①,已验像素级一致);②描边色取 AddStroke 默认黑(原工程 elide 了描边色=AE 建时默认,不可恢复)——0.8px 细线;③Scale Z=1 vs 原 0.853(2D 层 Z 无关);④Position 写 unified(0,0) vs 原 separated(0,0),值同;⑤kf linear 近似原 ease。终帧 render-pixel 归 ⑫。30fps。 |
