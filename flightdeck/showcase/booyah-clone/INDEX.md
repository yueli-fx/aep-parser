...
| ⑤ | プリコンポジション 1 | 🔶**待review**(双版本 AE-accept + DOM 对账 PASS) | `gen_precomp1.go`:**首遇 precomp 嵌套**——3 个 `NewPrecompLayer` 全引用 comp ①(staggered 三份 glitch)。L0 Position 2kf[778→1204]/L1 Opacity 13kf+居中/L2 静态居中;层 start 错峰 0.267/0.133/0.167 经 `SetStartTime`(post-reopen ldta edit;**坑**:precomp 是 embed-template clone,pre-reopen 的 scene `StartTime` 字段不生效,须 post-reopen SetStartTime)。**L1/L2 又撞 separated-position 读 0,0 陷阱**(comp ④ 同款)→ 显式居中。Go round-trip:3 层 srcID 全=comp ①、Position/Opacity/start 全等。**AE2020≡AE2025 接受**:5 comp 全在、⑤ 3 层不 drop、**AE DOM 三层 source 全绑到 comp ①**(precomp source-ID 解析正确,非 empty/wrong——[[precomp-layer-source-id-re]] 的失败面已排除)。verify.jsx 加 `l.source.name` dump。comp ① 像素已用户验收 → ⑤=三份其拷贝。终帧 render-pixel 归 ⑫。无新 API。 |
| ⑥ | シェイプの塊 | ⬜待建 | 7 层(需⑤) |
