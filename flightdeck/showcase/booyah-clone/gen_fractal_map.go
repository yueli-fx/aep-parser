// flightdeck/showcase/booyah-clone/gen_fractal_map.go — comp ③ "マップ用フラクタルノイズ".
...
func applyFractalNoise(layer *aep.Layer, orig *aep.Layer) {
	...
	for _, mn := range []string{
		...
		if _, err := aep.SetEffectParam(layer, fx, mn, toScalar(op.StaticValue)); err != nil {
			must(err)
		}
	}
	...
	if _, err := aep.AnimateEffectParamVec(layer, fx, "ADBE Fractal Noise-0013", vkfs); err != nil {
		must(err)
	}
	...
	evo, err := aep.SetEffectParam(layer, fx, "ADBE Fractal Noise-0023", 0.0)
	must(err)
