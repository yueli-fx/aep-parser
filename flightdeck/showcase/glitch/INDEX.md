...
| 文件 | 类型 | 说明 |
|---|---|---|
| `gen.go` | 生成器(Go, tracked) | 构建 `glitch.aep`（1920×1080，AE2020 target，2 comp：`TXT` 文字源 + `GLITCH` 主合成 8 层） |
| `render.jsx` | 渲染脚本(tracked) | AE `saveFrameToPng(1.5)` → `glitch.png`（8bpc + purge 缓存） |
| `glitch.aep` / `glitch.png` | 产出 (gitignored) | |

## 应看到

居中的「**GLITCH**」文字：字母**左缘青、右缘橙红**（RGB 色差），有横向切片撕裂错位，neon 辉光，叠细扫描线，**干净深色背景**（噪声仅作位移图源、不可见）。全 native，stock AE 即可渲（无需任何插件）。
