---
status: active
summary: 把整个 Booyah Glitch 真实工程(12 comp/61 层/~100 mask/wiggle 表达式/Curves)用咱们的 Go API 从零完整复刻,作为理解金标准检验。判据=结构保真(AE 接受 + DOM 读回值对账)+ 终帧渲染像素对照;范围=全工程 DAG 叶子优先逐 comp 验;方法=手写 per-comp 生成器、原工程仅当取值神谕、chunk 全由 API 重建(绝不 copy 字节)。
