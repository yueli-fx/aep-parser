...
- **Gradient Start/End Pt（方向）✅ DONE 2026-06-13**：翻案——「需 UI 授权」是错的，**JSX 能设 typed point 属性** `ADBE Vector Grad Start Pt`/`End Pt`（只有 stops XML 不能 author）。开 `v2_2_gradient_src.aep`（已带 stops）→ JSX 设 Start/End 对角线 → 重存 → AE 发射 slot。`lowerGradientFillNode` 覆写 Start/End Pt（Vec2 @cdat[0:16]，默认 [0,0]→[100,0] 无回归）。`GradientFillNode.SetStartPoint/SetEndPoint`。✅ `TestMGGradientDir_AEShipGate_*` 双版本渲染像素 PASS（red→blue 对角线 ramp，TL红/BR蓝/TR·BL紫；既有 gradient gate 不回归）。**deferred**：Grad Type(radial)/HiLite/G-Stroke 方向/direction read-back。详 `incidents/gradient-fill-write-re.md` § UPDATE 2026-06-13。
...
- ~~文本动画器（Text Animators，大坑）~~ ✅ **2026-06-15 首落地**（纠正：不前置 btdk——动画器住属性树 tdgp，与 btdk 正交）。`AddTextOpacityAnimator` + `AnimateTextRangeOffset`（offset 扫光逐字揭示），synthesis-insert vein，AE 2020+2025 双版本渲染 gate PASS。详 `incidents/text-animator-create-re.md`。按需扩 Position/Scale/Color 动画器。
- **矢量滤镜家族完成**（Trim/Repeater/RoundCorners/Offset/Merge/ZigZag 共 6 个 + Repeater 嵌套组，蓝本 7 次全绿）。
- **PolyStar（`ADBE Vector Shape - Star`）✅ DONE 2026-06-13**：新 from-scratch **形状**（非滤镜），套 Rect/Ellipse shape-body vein。`AddStar`/`StarNode`，模 Points/Position/Rotation/Inner·Outer Radius/Inner·Outer Roundess（match-name 对齐既有 read 侧 parse_shape.go）；Star Type 默认 Star elide（Polygon 暂搁，需 Type slot + 独立模板）。`templates/v2_2_shape_star_body.bin`。✅ `TestMGStar_AEShipGate_*` 双版本渲染像素 PASS（5 角星 OuterR=250/InnerR=100：5 tip 方向白·5 gap 方向暗·center 白·resave 读回；眼验金色五角星）。
- 剩 Twist / Pucker&Bloat / Wiggle 等同 vein 需求驱动可推。
