...
7. **交付准则（什么算「能用」）**：只有经 AE 双版本 ship-gate 实测 PASS 的能力、且仅在其 gate 覆盖的**规模 + 组合**边界内，才可对外宣称「能用 / 可交付」。三条认知红线：(a) **Go round-trip 0 警告 ≠ AE 接受**（Go parser 宽松，只验字节读回，不验 AE 引擎消化——`SetExpression` 栽在这）；(b) **`Stable` 有覆盖边界**，超出规模/组合（如 gate 只验 2 关键帧、你用 13 个空间关键帧）即退回未验证；(c) **组合 / 端到端是独立交付项**，单点 gate 不为「从零拼完整工程」背书。演示 / 示例 / 对用户的能力描述**禁止**混入未验证能力（混入必须当场标红）。本库擅长「读真实 .aep + 局部改 + 写回 / 单点结构性写」，**不擅长从零拼复杂工程**（from-scratch 缺省略 / 表达式不认 / 规模未测）。全文详 `checklists/delivery-contract.md`。
